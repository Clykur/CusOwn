...
const reserveRateLimit = enhancedRateLimit({ maxRequests: 20, windowMs: 60000, perIP: true, keyPrefix: 'slot_reserve' });
...
export async function POST(
  ...
  try {
    ...
    if (abuseCheck.blocked) {
      console.warn(`[ABUSE] Blocked slot reservation: ${abuseCheck.reason}, IP: ${clientIP}, User: ${userId?.substring(0, 8) || 'anonymous'}...`);
    ...
    if (!reserved) {
      console.warn(`[SECURITY] Slot reservation failed from IP: ${clientIP}, Slot: ${slotId.substring(0, 8)}...`);
