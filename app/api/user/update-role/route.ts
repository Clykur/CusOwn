...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!role || (role !== 'owner' && role !== 'customer')) {
      console.warn(`[SECURITY] Invalid role update request from IP: ${clientIP}, User: ${user.id.substring(0, 8)}...`);
    ...
    if (currentType === 'admin') {
      console.warn(`[SECURITY] Attempted role change from admin by IP: ${clientIP}, User: ${user.id.substring(0, 8)}...`);
      return errorResponse('Admin role cannot be changed', 403);
    }
    
    // SECURITY: Prevent escalation to admin
    if (role === 'admin') {
      console.warn(`[SECURITY] Attempted admin role escalation from IP: ${clientIP}, User: ${user.id.substring(0, 8)}...`);
    ...
    console.log(`[SECURITY] User role updated: IP: ${clientIP}, User: ${user.id.substring(0, 8)}..., From: ${currentType}, To: ${newType}`);
    ...
    return successResponse({ 
      user_type: newType,
      message: 'Role updated successfully. Redirect will be handled by the application based on your current state.'
    });
  } catch (error) {
