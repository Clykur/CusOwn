...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const closures = await dedupe(`closures:${businessId}`, () =>
      downtimeService.getBusinessClosures(businessId)
    );

    return successResponse(closures);
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to fetch closures';
