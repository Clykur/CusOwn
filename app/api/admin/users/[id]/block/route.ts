...
const ROUTE = 'POST /api/admin/users/[id]/block';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return successResponse(user, SUCCESS_MESSAGES.USER_BLOCKED);
  } catch (error) {
    if (error instanceof Error && error.message === 'User not found') {
      return errorResponse('User not found', 404);
    }
    const message = error instanceof Error ? error.message : ERROR_MESSAGES.USER_BLOCK_FAILED;
