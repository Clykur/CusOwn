...
export default function CustomerDashboardPage() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      console.log('[CUSTOMER_DASHBOARD] Starting to fetch bookings for user:', session.user.id.substring(0, 8) + '...');
      try {
        ...
        console.log('[CUSTOMER_DASHBOARD] API response status:', response.status, response.statusText);
          ...
          if (result.success) {
            ...
            for (const booking of bookingsData) {
              try {
                ...
                console.error(`Failed to generate secure URL for booking ${booking.booking_id}:`, error);
            ...
            const slotsPromises = bookingsData
              .filter((b: any) => b.slot && (b.salon || b.business) && (b.status === 'confirmed' || b.status === 'pending'))
              .map(async (b: any) => {
                try {
                  console.log('[CUSTOMER_DASHBOARD] Fetching slots for booking:', b.id?.substring(0, 8) + '...', 'business_id:', b.business_id?.substring(0, 8) + '...', 'date:', b.slot.date);
                  const slotsRes = await fetch(`/api/slots?salon_id=${b.business_id}&date=${b.slot.date}`, {
                    headers: {
                      'Authorization': `Bearer ${session.access_token}`,
                    },
                  });
                  ...
                  if (slotsResult.success) {
                    console.log('[CUSTOMER_DASHBOARD] Slots fetched for booking:', b.id?.substring(0, 8) + '...', 'count:', slotsResult.data?.length || 0);
                    return { bookingId: b.id, slots: slotsResult.data || [] };
                  } else {
                    console.warn('[CUSTOMER_DASHBOARD] Failed to fetch slots for booking:', b.id?.substring(0, 8) + '...', 'error:', slotsResult.error);
                  }
                } catch (err) {
                  console.error('[CUSTOMER_DASHBOARD] Exception fetching slots for booking:', b.id?.substring(0, 8) + '...', err);
                }
                return null;
              });
            ...
            console.log('[CUSTOMER_DASHBOARD] Slots map created with', Object.keys(newSlotsMap).length, 'entries');
        ...
        {loading ? (
          ...
          <>
            <div className="mb-6 sm:mb-8">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-4">
                <div>
                  <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-2">My Bookings</h1>
                  <p className="text-gray-600 text-base sm:text-lg">View and manage your appointments</p>
                </div>
                <div className="flex gap-2 sm:gap-3">
                  <button
              ...
              <div className="mb-8 bg-yellow-50 border-2 border-yellow-200 rounded-xl p-6">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <details className="mt-4">
                    <summary className="cursor-pointer font-semibold text-yellow-900">View Booking Details</summary>
            ...
            <div className="mb-6 sm:mb-8 bg-gradient-to-r from-black to-gray-800 rounded-2xl p-5 sm:p-6 md:p-8 shadow-lg">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-start gap-3 sm:gap-4">
                  ...
                  <div>
                    <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-white mb-1 sm:mb-2">Own a Business?</h3>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-12 md:p-16 text-center">
            ...
            <p className="text-gray-600 text-lg mb-8 max-w-md mx-auto">
              Start booking appointments to see them here. Find your favorite business and book a slot!
            </p>
            <Link
              href={ROUTES.CATEGORIES}
          ...
          <div className="grid gap-4 sm:gap-6 mb-8">
            {bookings.map((booking) => (
              <CustomerBookingCard
                ...
                secureUrl={secureBookingUrls.get(booking.booking_id) || ROUTES.BOOKING_STATUS(booking.booking_id)}
              />
            ))}
          </div>
        )}
