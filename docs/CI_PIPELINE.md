...
| **quality-gates**     | depcheck, ts-prune                                                           | Unused deps/exports (optional: continue-on-error) |
...
5. **Unit tests**: Run `npm run test:unit` then `npx vitest run --config vitest.config.mts --coverage`. Check coverage thresholds.
