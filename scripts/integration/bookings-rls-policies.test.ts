...
async function runBookingsRlsTests() {
  ...
  try {
    ...
    const customer1Client = await createAuthenticatedClient(
      customer1Email,
      TEST_PASSWORD,
    );
    const owner1Client = await createAuthenticatedClient(
      owner1Email,
      TEST_PASSWORD,
    );
    const owner2Client = await createAuthenticatedClient(
      owner2Email,
      TEST_PASSWORD,
    );
    ...
    if (customer1Bookings.error) {
      throw new Error(
        `Customer1 bookings select failed: ${customer1Bookings.error.message}`,
      );
    }
    ...
    if (
      !customer1BookingList.some((r: { id: string }) => r.id === booking1Id)
    ) {
    ...
    if (owner1Bookings.error) {
      throw new Error(
        `Owner1 bookings select failed: ${owner1Bookings.error.message}`,
      );
    }
    const owner1List = owner1Bookings.data ?? [];
    if (
      owner1List.length < 1 ||
      !owner1List.some((r: { id: string }) => r.id === booking1Id)
    ) {
      throw new Error(
    ...
    if (owner2Other.error) {
      throw new Error(
        `Owner2 select other business booking failed: ${owner2Other.error.message}`,
      );
    }
    if (owner2Other.data) {
      throw new Error(
        "Owner2 must not view bookings belonging to another owner business",
      );
    }
    ...
    if (customerUpdateOther && customerUpdateOther.length > 0) {
      throw new Error(
        "Owner must not modify bookings belonging to another business",
      );
    }
    ...
    if (owner1UpdateOwn && owner1UpdateOwn.length === 0) {
      throw new Error(
        "Owner should be able to update bookings of their business",
      );
    }
    ...
    if (adminAll.error) {
      throw new Error(
        `Admin bookings select failed: ${adminAll.error.message}`,
      );
    }
    const adminList = adminAll.data ?? [];
    if (adminList.length !== 2) {
      throw new Error(
        `Admin should see all bookings (2), got ${adminList.length}`,
      );
    }
    ...
    if (!adminUpdateData || adminUpdateData.length === 0) {
      throw new Error(
        "Admin should be able to update bookings from any business",
      );
    }
    ...
    if (anonList.length > 0) {
      throw new Error(
        `Anonymous users must not read bookings, got ${anonList.length}`,
      );
    }
