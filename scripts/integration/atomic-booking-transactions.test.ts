  ...
  try {
    ...
    await runner.runTest("ATOMIC 1: Single user booking succeeds", async () => {
      ...
      if (error || !result || !result.success) {
        throw new Error(
          `Booking creation failed: ${error?.message || result?.error}`,
        );
      }
      ...
      console.log(
        `   ✅ Booking created: ${result.booking_id.substring(0, 8)}...`,
      );
    ...
    await runner.runTest(
      "ATOMIC 2: Concurrent bookings - only one succeeds",
      async () => {
        const business = await getRandomBusiness();
        const slot = await getRandomAvailableSlot(business.id);
        cleanup.slots.push(slot.id);
        ...
        if (success1 && success2) {
          throw new Error(
            "Both bookings succeeded - race condition not prevented!",
          );
        }
        ...
        if (!bookings || bookings.length !== 1) {
          ...
          throw new Error(
            `Expected 1 booking for this test, found ${bookings?.length || 0}. Total bookings for slot: ${allBookings?.length || 0}`,
          );
        }
        ...
        console.log(
          `   ✅ Only one booking succeeded (${success1 ? "Booking 1" : "Booking 2"})`,
        );
        console.log(
          `   ✅ Failed booking error: ${failedResult.data?.error || failedResult.error?.message}`,
        );
        console.log(`   ✅ Slot status: ${updatedSlot.status}`);
    ...
    // ============================================
    await runner.runTest(
      "ATOMIC 3: Booking on already reserved slot fails",
      async () => {
        const business = await getRandomBusiness();
        const slot = await getRandomAvailableSlot(business.id);
        cleanup.slots.push(slot.id);
        ...
        const baseTimestamp = Date.now();
        const bookingId1 = `TEST-${baseTimestamp}-1`;
        const { data: result1 } = await supabase.rpc(
          "create_booking_atomically",
          {
            p_business_id: business.id,
            p_slot_id: slot.id,
        ...
        if (slotAfterFirst?.status !== "reserved") {
          throw new Error(
            `Slot not reserved after first booking: ${slotAfterFirst?.status}`,
          );
        }
        ...
        const bookingId2 = `TEST-${baseTimestamp}-2`;
        const { data: result2, error } = await supabase.rpc(
          "create_booking_atomically",
          {
            p_business_id: business.id,
            p_slot_id: slot.id,
        ...
        if (result2?.success) {
          ...
          if (slotAfterSecond?.status === "reserved") {
            ...
            console.log(
              `   ⚠️  Slot reservation expired between attempts (status: ${slotAfterSecond?.status})`,
            );
          ...
          if (
            !result2?.error ||
            (!result2.error.includes("reserved") &&
              !result2.error.includes("Slot"))
          ) {
            throw new Error(
              `Expected reservation error, got: ${result2?.error || error?.message}`,
            );
          }
        }
        ...
        if (!bookings || bookings.length === 0) {
          throw new Error(
            `Expected at least 1 booking from this test, found ${bookings?.length || 0}`,
          );
        }

        if (result2?.success) {
          console.log(
            `   ✅ Second booking succeeded (reservation expired): ${result2.error || "OK"}`,
          );
          console.log(
            `   ✅ Total bookings from this test: ${bookings.length} (expected due to expiry)`,
          );
        } else {
          if (bookings.length !== 1) {
            ...
            const { data: allBookings } = await supabase
              ...
              .eq("slot_id", slot.id);
            throw new Error(
              `Expected 1 booking from this test, found ${bookings.length}. Total bookings for slot: ${allBookings?.length || 0}`,
            );
          }
          console.log(
            `   ✅ Second booking correctly rejected: ${result2.error}`,
          );
    ...
    // ============================================
    await runner.runTest(
      "ATOMIC 4: Booking on already booked slot fails",
      async () => {
        const business = await getRandomBusiness();
        const slot = await getRandomAvailableSlot(business.id);
        cleanup.slots.push(slot.id);
        ...
        const { data: result2 } = await supabase.rpc(
          "create_booking_atomically",
          {
            p_business_id: business.id,
            p_slot_id: slot.id,
    ...
    // ============================================
    await runner.runTest(
      "ATOMIC 5: Booking on suspended business fails",
      async () => {
        const business = await getRandomBusiness();
        const slot = await getRandomAvailableSlot(business.id);
        cleanup.slots.push(slot.id);
    ...
    // ============================================
    await runner.runTest(
      "ATOMIC 6: Booking with slot from wrong business fails",
      async () => {
    ...
    await runner.runTest("ATOMIC 7: No partial writes on failure", async () => {
      ...
      if (finalSlot?.status !== initialSlot?.status) {
        throw new Error(
          `Slot state changed: ${initialSlot?.status} → ${finalSlot?.status}`,
        );
      }

      // Verify no booking created
      const finalBookingCount =
        (
          await supabase
            .from("bookings")
            .select("id", { count: "exact", head: true })
      ...
      if (finalBookingCount !== initialBookingCount) {
        throw new Error(
          `Booking count changed: ${initialBookingCount} → ${finalBookingCount}`,
        );
      }
