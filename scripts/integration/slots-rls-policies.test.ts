...
async function runSlotsRlsTests() {
  ...
  try {
    ...
    if (customerSlots.error) {
      throw new Error(
        `Customer slots select failed: ${customerSlots.error.message}`,
      );
    }
    ...
    if (
      customerIds.includes(slotSuspendedBiz) ||
      customerIds.includes(slotDeletedBiz)
    ) {
      throw new Error(
        "Customer must not see slots of suspended or deleted business",
      );
    }
    ...
    if (owner1UpdateOwn) {
      throw new Error(
        `Owner must be able to update own business slot: ${owner1UpdateOwn.message}`,
      );
    }
    ...
    if (owner2UpdateOther && owner2UpdateOther.length > 0) {
      throw new Error(
        "Owner must not modify slots belonging to another business",
      );
    }
    ...
    if (adminAll.data?.length !== 5) {
      throw new Error(
        `Admin should see all 5 slots, got ${adminAll.data?.length}`,
      );
    }
    ...
    if (adminUpdateBooked) {
      throw new Error(
        `Admin must be able to update booked slot: ${adminUpdateBooked.message}`,
      );
    }
    ...
    if (adminUpdate) {
      throw new Error(
        `Admin must have unrestricted update: ${adminUpdate.message}`,
      );
    }
    ...
    if (serviceDelete) {
      throw new Error(
        `Service role must be able to delete slots: ${serviceDelete.message}`,
      );
    }
