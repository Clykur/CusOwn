...
async function runDatabaseMigrationsTests(): Promise<void> {
  ...
  await runner.runTest(
    "core tables exist (businesses, slots, bookings)",
    async () => {
  ...
  await runner.runTest(
    "slots.business_id FK: table has expected structure",
    async () => {
  ...
  await runner.runTest(
    "bookings.slot_id and business_id columns exist",
    async () => {
      const { data, error } = await supabase
        .from("bookings")
  ...
  await runner.runTest(
    "audit_logs table exists when migrations applied",
    async () => {
