...
async function runDatabaseIndexesTests(): Promise<void> {
  ...
  await runner.runTest(
    "booking indexes exist and basic queries succeed",
    async () => {
      ...
      if (
        q1Error &&
        !q1Error.message.includes('relation "bookings" does not exist')
      ) {
        throw q1Error;
      }
      ...
      if (
        q2Error &&
        !q2Error.message.includes('relation "bookings" does not exist')
      ) {
        throw q2Error;
      }
    },
  ...
  await runner.runTest(
    "discovery indexes exist and discovery query succeeds",
    async () => {
      ...
      if (
        error &&
        !error.message.includes("function public.search_businesses_ranked")
      ) {
        throw error;
      }
    },
  ...
  await runner.runTest(
    "audit log indexes exist and basic queries succeed",
    async () => {
      const auditIndexes = [
      ...
      if (
        q1Error &&
        !q1Error.message.includes('relation "audit_logs" does not exist')
      ) {
        throw q1Error;
      }
      ...
      if (
        q2Error &&
        !q2Error.message.includes('relation "audit_logs" does not exist')
      ) {
        throw q2Error;
      }
    },
  ...
  await runner.runTest(
    ...
    async () => {
      ...
      const { error: qErr } = await supabase
        ...
        .order("created_at", { ascending: false })
        .limit(5);
      if (qErr && !qErr.message.includes('relation "bookings" does not exist'))
        throw qErr;
    },
  );
  ...
  await runner.runTest(
    ...
    async () => {
      ...
      if (
        qErr &&
        !qErr.message.includes('relation "businesses" does not exist')
      )
        throw qErr;
    },
  );
  ...
  await runner.runTest(
    ...
    async () => {
      ...
      if (
        qErr &&
        !qErr.message.includes('relation "audit_logs" does not exist')
      )
        throw qErr;
    },
  );
