...
vi.mock("@/services/service.service", () => ({
  serviceService: {
    ...
    calculateTotalDuration: (...args: unknown[]) =>
      mockCalculateTotalDuration(...args),
    calculateTotalPrice: (...args: unknown[]) =>
      mockCalculateTotalPrice(...args),
  },
}));
...
describe("booking.service", () => {
  ...
  describe("prepareCreateBookingParams", () => {
    ...
    it("calls validateServices and sets duration/price when serviceIds provided", async () => {
      ...
      expect(mockValidateServices).toHaveBeenCalledWith(
        ["svc1"],
        validInput.salon_id,
      );
      ...
      expect(out.p_services_count).toBe(1);
      expect(out.p_service_data).toEqual([
        { service_id: "svc1", price_cents: 500 },
      ]);
    });
    ...
    it("throws when booking_id collision after max attempts", async () => {
      ...
      await expect(
        bookingService.prepareCreateBookingParams(validInput, null),
      ).rejects.toThrow(ERROR_MESSAGES.DATABASE_ERROR);
    });

    it("throws when validateServices throws", async () => {
      mockValidateServices.mockRejectedValue(
        new Error("Invalid or inactive service"),
      );
      await expect(
