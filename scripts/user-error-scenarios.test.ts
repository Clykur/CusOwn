...
async function testErrorScenarios() {
  ...
  try {
    await runner.runTest('STEP 1: Try to book suspended business', async () => {
      ...
      await supabase.from('businesses').update({ suspended: true, suspended_at: new Date().toISOString() }).eq('id', business.id);
      ...
      const { data, error } = await supabase.rpc('create_booking_atomically', {
        p_business_id: business.id, p_slot_id: slot.id, p_customer_name: 'Test Customer',
        p_customer_phone: '+919876543210', p_booking_id: bookingId, p_customer_user_id: null,
      ...
      await supabase.from('businesses').update({ suspended: false, suspended_at: null }).eq('id', business.id);
    });
  } finally {
...
if (require.main === module) {
  testErrorScenarios().then(() => process.exit(0)).catch((error) => { console.error('Test failed:', error); process.exit(1); });
}
