...
async function testOwnerJourney() {
  ...
  try {
    ...
    await runner.runTest('STEP 2: Owner views dashboard (no business)', async () => {
      ...
      const { data: businesses } = await supabase.from('businesses').select('*').eq('owner_user_id', owner.id);
    ...
    await runner.runTest('STEP 4: Owner views their business', async () => {
      ...
      const { data, error } = await supabase.from('businesses').select('*').eq('owner_user_id', owner.id).single();
...
if (require.main === module) {
  testOwnerJourney().then(() => process.exit(0)).catch((error) => { console.error('Test failed:', error); process.exit(1); });
}
