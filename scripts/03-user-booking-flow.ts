  ...
  try {
    ...
    await runner.runTest('STEP 3: User creates booking', async () => {
      ...
      const { data, error } = await supabase.rpc('create_booking_atomically', {
        p_business_id: business.id, p_slot_id: slot.id, p_customer_name: 'Booking Test Customer',
        p_customer_phone: '+919876543210', p_booking_id: bookingIdStr, p_customer_user_id: customer.id,
        p_total_duration_minutes: 30, p_total_price_cents: 1000, p_services_count: 1, p_service_data: null,
      });
    ...
    await runner.runTest('STEP 4: Booking is confirmed', async () => {
      ...
      const { data, error } = await supabase.rpc('confirm_booking_atomically', { p_booking_id: bookingId, p_actor_id: null });
      if (error || !data?.success) throw new Error(error?.message || data?.error || 'Booking confirmation failed');
      ...
      console.log(`   ⏰ Time: ${booking?.slot?.start_time || 'N/A'} - ${booking?.slot?.end_time || 'N/A'}`);
...
if (require.main === module) {
  testBookingFlow().then(() => process.exit(0)).catch((error) => { console.error('Test failed:', error); process.exit(1); });
}
