  ...
  try {
    ...
    await runner.runTest('E2E 1: Happy path - complete booking flow', async () => {
      ...
      if (paymentError && paymentError.message.includes('does not exist')) {
        ...
        if (confirmResult && confirmResult.success) {
          ...
          console.log(`   ✅ Final state: Booking=${finalBooking.status}, Slot=${finalSlot.status}, Payment=${finalPayment.status}`);
        } else {
          console.log(`   ⚠️  Payment confirmation function not available: ${confirmResult?.error}`);
        }
      }
    });
    ...
    await runner.runTest('E2E 2: Negative flow - payment failure', async () => {
      ...
      if (slotAfterFailure?.status !== 'reserved') {
        throw new Error(`Slot should remain reserved after payment failure: ${slotAfterFailure?.status}`);
      }
    ...
    await runner.runTest('E2E 3: Concurrent bookings - only one succeeds', async () => {
      ...
      if (bookings?.length !== 1) {
        ...
        throw new Error(`Expected 1 booking from this test, found ${bookings?.length || 0}. Total bookings for slot: ${allBookings?.length || 0}. Found IDs: ${foundBookingIds}. Test IDs: ${bookingIds.join(', ')}`);
      }
