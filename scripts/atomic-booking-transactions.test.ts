...
async function testAtomicBookingTransactions() {
  ...
  try {
    ...
    await runner.runTest('ATOMIC 2: Concurrent bookings - only one succeeds', async () => {
      ...
      await supabase.from('bookings').delete().eq('slot_id', slot.id).like('booking_id', 'TEST-%');
    ...
    await runner.runTest('ATOMIC 3: Booking on already reserved slot fails', async () => {
      ...
      if (result2?.success) {
        ...
        if (
          !result2?.error ||
          (!result2.error.includes('reserved') && !result2.error.includes('Slot'))
        ) {
    ...
    await runner.runTest('ATOMIC 5: Booking on suspended business fails', async () => {
      ...
      // Suspend business
      await supabase.from('businesses').update({ suspended: true }).eq('id', business.id);
      ...
      // Restore business
      await supabase.from('businesses').update({ suspended: false }).eq('id', business.id);
