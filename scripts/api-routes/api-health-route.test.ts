...
export async function runApiHealthRouteTests(): Promise<void> {
  ...
  await runAsyncTest(
    ...
    async () => {
      ...
      assert(
        typeof body.data?.checks?.database === "string",
        "Expected checks.database",
      );
      assert(
        typeof body.data?.checks?.timestamp === "string",
        "Expected checks.timestamp",
      );
    },
  );
}
...
export async function runApiOpenApiRouteTests(): Promise<void> {
  ...
  await runAsyncTest(
    "GET /api/openapi returns 200 and YAML content-type",
    async () => {
...
export async function runApiCsrfTokenRouteTests(): Promise<void> {
  ...
  await runAsyncTest(
    "GET /api/csrf-token returns 200 and success with token",
    async () => {
  ...
  await runAsyncTest(
    "GET /api/csrf-token returns same token when cookie present",
    async () => {
      const { GET } = await import("../../app/api/csrf-token/route");
      ...
      assert(
        ...
        `Expected token from cookie, got ${body.data?.token}`,
      );
    },
  );
}
