...
vi.mock("@/lib/security/rate-limit-api.security", () => ({
  bookingRateLimitEnhanced: (...args: unknown[]) =>
    mockBookingRateLimitEnhanced(...args),
}));
...
vi.mock("@/services/booking.service", () => ({
  bookingService: {
    ...
    prepareCreateBookingParams: (...args: unknown[]) =>
      mockPrepareCreateBookingParams(...args),
  },
}));
...
vi.mock("@/lib/booking-retry", () => ({
  withBookingRetry: (opts: { fn: () => Promise<unknown> }) =>
    mockWithBookingRetry(opts),
}));
...
describe("POST /api/bookings", () => {
  ...
  beforeEach(() => {
    ...
    mockWithBookingRetry.mockImplementation(
      (opts: { fn: () => Promise<unknown> }) => opts.fn(),
    );
  });
