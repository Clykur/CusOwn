...
async function testAdminAsCustomer() {
  ...
  try {
    ...
    await runner.runTest("STEP 1: Admin logs in", async () => {
      admin = await getOrCreateTestUser(
        `test-admin-customer-${Date.now()}@test.com`,
        "owner",
      );
      await supabase
        .from("user_profiles")
    ...
    await runner.runTest("STEP 2: Admin views customer dashboard", async () => {
      ...
      const { data: customerBookings, error } = await supabase
        .from("bookings")
        .select(
          "*, business:business_id(salon_name), slot:slot_id(date, start_time)",
        )
      ...
      console.log(
        `   Admin has ${customerBookings?.length || 0} bookings as customer`,
      );
    });
