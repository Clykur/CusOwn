...
async function testSlotServiceComprehensive() {
  ...
  try {
    ...
    await runner.runTest(
      "SLOT SERVICE 1: Generate slots for business",
      async () => {
        ...
        console.log(
          `   ✅ Generated ${slots?.length || 0} slots for ${dateStr}`,
        );
        console.log(`   📊 Business: ${business.salon_name}`);
        console.log(
          `   ⏰ Hours: ${business.opening_time} - ${business.closing_time}`,
        );
    ...
    // ============================================
    await runner.runTest(
      "SLOT SERVICE 2: Get available slots (lazy generation)",
      async () => {
        const business = await getRandomBusiness();
        ...
        if (slots.length > 0) {
          console.log(
            `   ⏰ First slot: ${slots[0].start_time} - ${slots[0].end_time}`,
          );
          console.log(
    ...
    await runner.runTest("SLOT SERVICE 3: Get slot by ID", async () => {
      ...
      console.log(
        `   ⏰ Time: ${retrievedSlot.start_time} - ${retrievedSlot.end_time}`,
      );
    ...
    await runner.runTest("SLOT SERVICE 5: Release reserved slot", async () => {
      ...
      if (releasedSlot?.status !== "available") {
        throw new Error(
          `Expected available status, got ${releasedSlot?.status}`,
        );
      }
      ...
      console.log(
        `   🔓 Reserved Until: ${releasedSlot.reserved_until || "null"}`,
      );
    });
    ...
    await runner.runTest("SLOT SERVICE 6: Book reserved slot", async () => {
      ...
      console.log(
        `   🔒 Reserved Until: ${bookedSlot.reserved_until || "null"}`,
      );
    });
    ...
    // ============================================
    await runner.runTest(
      "SLOT SERVICE 7: Release expired reservations",
      async () => {
        const business = await getRandomBusiness();
        ...
        console.log(
          `   📊 Expired slot status: ${slot1After?.status} (should be available)`,
        );
        console.log(
          `   📊 Active slot status: ${slot2After?.status} (should be reserved)`,
        );

        if (slot1After?.status !== "available") {
          throw new Error(
            `Expired slot should be available, got ${slot1After?.status}`,
          );
        }
      },
    );
    ...
    // ============================================
    await runner.runTest(
      "SLOT SERVICE 9: Invalid state transition prevention",
      async () => {
        const business = await getRandomBusiness();
        const slot = await getRandomAvailableSlot(business.id);
        cleanup.slots.push(slot.id);
        ...
        try {
          ...
          if (reserved) {
            throw new Error(
              "Should not be able to reserve an already booked slot",
            );
          }
          ...
          if (errorMsg.includes("Cannot") || errorMsg.includes("Invalid")) {
            console.log(
              `   ✅ Correctly rejected invalid transition: ${errorMsg}`,
            );
          } else {
            throw error;
          }
    ...
    await runner.runTest("SLOT SERVICE 10: Past slot filtering", async () => {
      ...
      console.log(
        `   📊 Past slots filtered: ${pastSlots.length === 0 ? "Yes" : "No"}`,
      );
      console.log(
      ...
      if (
        pastSlots.length > 0 &&
        slots.some((s: any) => s.status !== "booked")
      ) {
        console.log(
          `   ⚠️  Warning: ${pastSlots.length} past slots found (may be booked slots)`,
        );
      }
    });
    ...
    // ============================================
    await runner.runTest(
      "SLOT SERVICE 11: Slot generation skips existing slots",
      async () => {
        const business = await getRandomBusiness();
        const tomorrow = new Date();
        tomorrow.setDate(tomorrow.getDate() + 1);
        ...
        if (firstCount !== secondCount) {
          throw new Error(
            `Expected same count, got ${firstCount} vs ${secondCount}`,
          );
        }
      },
    );
