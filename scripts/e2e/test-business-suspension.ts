...
async function testBusinessSuspension() {
  ...
  try {
    await runner.runTest("Get real business", async () => {
      ...
      console.log(
        `   Business: ${business.salon_name} (${business.id.substring(0, 8)}...)`,
      );
    ...
    await runner.runTest(
      "Verify suspended business excluded from search",
      async () => {
        if (!testBusinessId) throw new Error("No business ID");
    ...
    await runner.runTest(
      "Verify suspended business cannot receive bookings",
      async () => {
        if (!testBusinessId) throw new Error("No business ID");
        ...
        if (data?.error?.includes("suspended")) {
          ...
          throw new Error(
            `Expected suspension error, got: ${data?.error || error?.message}`,
          );
        }
      },
    );
    ...
    await runner.runTest(
      "Verify getSalonByBookingLink excludes suspended",
      async () => {
        if (!testBusinessId) throw new Error("No business ID");
        ...
        console.log(
          `   Suspended business correctly excluded from getSalonByBookingLink`,
        );
      },
    );
  } finally {
