...
async function testE2EFlows() {
  ...
  try {
    ...
    await runner.runTest(
      "E2E 1: Happy path - complete booking flow",
      async () => {
        ...
        if (slotAfterBooking?.status !== "reserved") {
          throw new Error(
            `Step 1: Slot not reserved: ${slotAfterBooking?.status}`,
          );
        }
        ...
        if (paymentError && paymentError.message.includes("does not exist")) {
          ...
          throw new Error(
            `Step 2: Payment creation failed: ${paymentError?.message}`,
          );
        } else {
          ...
          if (confirmResult && confirmResult.success) {
            ...
            if (finalBooking?.status !== "confirmed") {
              throw new Error(
                `Step 4: Booking not confirmed: ${finalBooking?.status}`,
              );
            }
            ...
            if (finalPayment?.status !== "completed") {
              throw new Error(
                `Step 4: Payment not completed: ${finalPayment?.status}`,
              );
            }
    ...
    await runner.runTest("E2E 2: Negative flow - payment failure", async () => {
      ...
      if (paymentError && paymentError.message.includes("does not exist")) {
        console.log(
          `   ⚠️  Payments table not found, skipping payment failure test`,
        );
        return;
      }
      ...
      if (booking?.status !== "pending") {
        throw new Error(
          `Booking should remain pending after payment failure: ${booking?.status}`,
        );
      }
    ...
    // ============================================
    await runner.runTest(
      "E2E 3: Concurrent bookings - only one succeeds",
      async () => {
        const business = await getRandomBusiness();
        const slot = await getRandomAvailableSlot(business.id);
        cleanup.slots.push(slot.id);
        ...
        if (bookings?.length !== 1) {
          ...
          const foundBookingIds =
            bookings?.map((b) => b.booking_id).join(", ") || "none";
          throw new Error(
        ...
        console.log(
          `   ✅ Concurrent bookings: ${successful} succeeded, ${5 - successful} failed`,
        );
