...
async function testConcurrentOperations() {
  ...
  try {
    await runner.runTest(
      "STEP 1: Multiple users compete for same slot",
      async () => {
        ...
        try {
          ...
          throw new Error(
            `No available slots found for concurrent test: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
        ...
        failed.forEach((f) => {
          const reason = f.reason.includes("reserved")
            ? "Slot already reserved"
            : f.reason.includes("booked")
              ? "Slot already booked"
              : f.reason.includes("failed")
                ? "Reservation failed"
                : "Other error";
        ...
        console.log(
          `      ❌ Failed: ${concurrentRequests - successful.length}`,
        );
        console.log(
        ...
        if (successful.length !== 1) {
          throw new Error(
            `Expected exactly 1 successful booking, got ${successful.length}`,
          );
        }
        ...
        console.log(
          `   🔐 Database locking (FOR UPDATE) prevented double-booking`,
        );
        console.log(
          `   ⚡ All ${concurrentRequests} requests processed atomically`,
        );
      },
    );
  } finally {
