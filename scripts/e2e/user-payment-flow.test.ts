...
async function testPaymentFlow() {
  ...
  try {
    await runner.runTest(
      "STEP 1: User creates booking requiring payment",
      async () => {
        customer = await getOrCreateTestUser(
          `test-payment-${Date.now()}@test.com`,
          "customer",
        );
    ...
    await runner.runTest("STEP 3: Payment succeeds", async () => {
      ...
      console.log(
        `   💰 Amount Paid: ₹${((updatedPayment?.amount_cents || 0) / 100).toFixed(2)}`,
      );
      if (updatedPayment?.verified_at) {
        console.log(
          `   ⏰ Verified At: ${new Date(updatedPayment.verified_at).toLocaleString()}`,
        );
      }
    });
  } finally {
