...
async function testEdgeCases() {
  ...
  try {
    ...
    await runner.runTest(
      "EDGE CASE 2: Queue-based workflow execution",
      async () => {
        ...
        if (JSON.stringify(results) !== JSON.stringify(expectedOrder)) {
          throw new Error(
            `Workflow execution order incorrect: ${results.join(" -> ")}`,
          );
        }
        ...
        console.log(
          `   ⏱️  Completed actions: ${workflow.getCompleted().length}`,
        );
      },
    );
    ...
    // ============================================
    await runner.runTest(
      "EDGE CASE 3: Multiple concurrent booking attempts",
      async () => {
        ...
        const customers = await Promise.all(
          Array.from({ length: 5 }, (_, i) =>
            getOrCreateTestUser(
              `concurrent-edge-${Date.now()}-${i}@test.com`,
              "customer",
            ),
          ),
        );
        ...
        if (successful !== 1) {
          throw new Error(
            `Expected exactly 1 successful booking, got ${successful}`,
          );
        }
        ...
        console.log(
          `   📊 Attempts: 5, Successful: ${successful}, Failed: ${5 - successful}`,
        );
      },
    );
    ...
    // ============================================
    await runner.runTest(
      "EDGE CASE 4: State tracking across operations",
      async () => {
        const customer = await getOrCreateTestUser(
          `edge-state-${Date.now()}@test.com`,
          "customer",
        );
        const business = await getRandomBusiness();
        const slot = await getRandomAvailableSlot(business.id);
        ...
        if (error || !data?.success) {
          throw new Error(
            `Booking creation failed: ${error?.message || data?.error}`,
          );
        }
        ...
        if (
          !stateTracker.has("booking_id") ||
          stateTracker.get("booking_status") !== "pending"
        ) {
        ...
        console.log(
          `   📊 Tracked keys: ${Array.from(stateTracker.getAll().keys()).join(", ")}`,
        );
        console.log(
          `   📋 History entries: ${stateTracker.getHistory().length}`,
        );
      },
    );
    ...
    // ============================================
    await runner.runTest(
      "EDGE CASE 5: Invalid state transition prevention",
      async () => {
        const customer = await getOrCreateTestUser(
          `edge-invalid-${Date.now()}@test.com`,
          "customer",
        );
        const business = await getRandomBusiness();
        const slot = await getRandomAvailableSlot(business.id);
        ...
        if (confirmError || !confirmData?.success) {
          throw new Error(
            `Booking confirmation failed: ${confirmError?.message || confirmData?.error}`,
          );
        }
    ...
    // ============================================
    await runner.runTest(
      "EDGE CASE 6: Boundary condition testing",
      async () => {
    ...
    await runner.runTest("EDGE CASE 7: Data integrity validation", async () => {
      const customer = await getOrCreateTestUser(
        `edge-integrity-${Date.now()}@test.com`,
        "customer",
      );
      const business = await getRandomBusiness();
      const slot = await getRandomAvailableSlot(business.id);
    ...
    // ============================================
    await runner.runTest(
      "EDGE CASE 8: Error recovery and rollback",
      async () => {
        const customer = await getOrCreateTestUser(
          `edge-recovery-${Date.now()}@test.com`,
          "customer",
        );
        const business = await getRandomBusiness();
        ...
        console.log(
          `   🚫 Error handled: ${error?.message || data?.error || "Unknown"}`,
        );
      },
    );
  } finally {
