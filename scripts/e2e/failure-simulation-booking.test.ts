...
async function simulateDatabaseDeadlock(runner: TestRunner) {
  await runner.runTest(
    "failure: database deadlock retried safely",
    async () => {
      ...
      const retries = await metricsService.getCount(
        METRICS_BOOKING_DEADLOCK_RETRY_TOTAL,
      );
      if (retries <= 0) {
        throw new Error(
          "Expected METRICS_BOOKING_DEADLOCK_RETRY_TOTAL to increment",
        );
      }
    },
  );
...
async function simulateServiceOutage(runner: TestRunner) {
  await runner.runTest(
    "failure: upstream service outage does not corrupt booking",
    async () => {
      const business = await getRandomBusiness();
      const slot = await getRandomAvailableSlot(business.id);
      ...
      const res = await fetch(`${apiBase}/api/payments/verify`, {
        ...
        console.error(
          "verify request failed (expected in outage simulation):",
          e,
        );
        return null as any;
      });
      ...
      if (
        !["pending", "confirmed", "cancelled", "rejected"].includes(
          bookingRow.status,
        )
      ) {
        throw new Error(
          `Booking in unexpected status after outage: ${bookingRow.status}`,
        );
      }
...
async function simulateNetworkTimeout(runner: TestRunner) {
  await runner.runTest(
    "failure: client-side timeout does not corrupt booking",
    async () => {
      const apiBase =
        process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000";
      ...
      const attempts = await metricsService.getCount(
        METRICS_OBSERVABILITY_BOOKING_ATTEMPT_TOTAL,
      );
      void attempts;
    },
