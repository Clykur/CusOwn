...
async function testCustomerJourney() {
  ...
  try {
    ...
    await runner.runTest("STEP 1: Customer logs in", async () => {
      customer = await getOrCreateTestUser(
        `test-customer-${Date.now()}@test.com`,
        "customer",
      );
    ...
    await runner.runTest("STEP 4: View available slots", async () => {
      ...
      try {
        ...
        console.log(
          `   Found available slot: ${slot.date} ${slot.start_time} - ${slot.end_time}`,
        );
        ...
        for (const dateStr of dates) {
          ...
          if (!queryError && data && data.length > 0) {
            console.log(
              `   Found ${data.length} available slots for ${dateStr}`,
            );
            slotsFound = true;
    ...
    await runner.runTest("STEP 5: Customer creates booking", async () => {
      const business = (global as any).selectedBusiness;
      const slot =
        (global as any).selectedSlot ||
        (await getRandomAvailableSlot(business.id));
      ...
      if (error || !data?.success) {
        throw new Error(
          error?.message || data?.error || "Booking creation failed",
        );
      }
    ...
    await runner.runTest("STEP 6: Customer views their bookings", async () => {
      ...
      const myBooking = data?.find(
        (b) => b.id === (global as any).customerBookingId,
      );
      if (!myBooking) {
      ...
      if (myBooking) {
        ...
        console.log(
          `      Business: ${myBooking.business?.salon_name || "N/A"}`,
        );
    ...
    await runner.runTest("STEP 7: Customer checks booking status", async () => {
      ...
      const { data, error } = await supabase
        .from("bookings")
        .select(
          "*, business:business_id(salon_name), slot:slot_id(date, start_time, end_time)",
        )
        .eq("id", bookingId)
        .single();
