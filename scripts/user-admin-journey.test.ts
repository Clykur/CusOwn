#!/usr/bin/env ts-node
import { supabase, TestRunner, getOrCreateTestUser, getRandomBusiness, simulateUserAction } from './test-utils';
async function testAdminJourney() {
  ...
  try {
    ...
    await runner.runTest('STEP 2: Admin views dashboard', async () => {
      ...
      const { data: businesses } = await supabase.from('businesses').select('id').eq('suspended', false);
    ...
    await runner.runTest('STEP 3: Admin views all businesses', async () => {
      ...
      const { data, error } = await supabase.from('businesses').select('id, salon_name, suspended').order('created_at', { ascending: false }).limit(10);
      if (error) throw new Error(`Failed to fetch businesses: ${error.message}`);
      console.log(`   Found ${data?.length || 0} businesses`);
    });
  } finally {
    if (admin) await supabase.from('user_profiles').update({ user_type: 'owner' }).eq('id', admin.id);
  }
  runner.printSummary();
  return runner.getResults();
}
if (require.main === module) {
  testAdminJourney().then(() => process.exit(0)).catch((error) => { console.error('Test failed:', error); process.exit(1); });
}
