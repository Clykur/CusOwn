...
async function testPaymentSafety() {
  ...
  try {
    ...
    await runner.runTest(
      "PAYMENT 1: Payment idempotency key prevents duplicates",
      async () => {
    ...
    // ============================================
    await runner.runTest(
      "PAYMENT 2: Payment attempts tracked on failure",
      async () => {
        const business = await getRandomBusiness();
        const slot = await getRandomAvailableSlot(business.id);
        cleanup.slots.push(slot.id);
        ...
        try {
          ...
          console.log(
            `   ✅ Payment attempts tracked: ${attempts.length} attempt(s)`,
          );
          console.log(
            `   ✅ Last attempt status: ${attempts[attempts.length - 1].status}`,
          );
          console.log(
            `   ✅ Error message: ${attempts[attempts.length - 1].error_message}`,
          );
        } catch (error: any) {
          if (error.message.includes("does not exist")) {
            console.log(
              `   ⚠️  Payment attempts table not found, skipping detailed check`,
            );
          } else {
            throw error;
          }
    ...
    // ============================================
    await runner.runTest(
      "PAYMENT 3: Duplicate webhook does not double-confirm",
      async () => {
        const business = await getRandomBusiness();
        const slot = await getRandomAvailableSlot(business.id);
        cleanup.slots.push(slot.id);
        ...
        try {
          ...
          if (booking1?.status !== booking2?.status) {
            throw new Error(
              `Booking state changed on duplicate webhook: ${booking1?.status} → ${booking2?.status}`,
            );
          }
          ...
          if (
            ...
            console.log(
              `   ⚠️  Payment service method not available, skipping detailed check`,
            );
          } else {
            throw error;
          }
    ...
    // ============================================
    await runner.runTest(
      "PAYMENT 4: Payment and booking state consistency",
      async () => {
        const business = await getRandomBusiness();
        const slot = await getRandomAvailableSlot(business.id);
        cleanup.slots.push(slot.id);
        ...
        if (initialBooking?.status !== "pending") {
          throw new Error(
            `Initial booking status wrong: ${initialBooking?.status}`,
          );
        }
        ...
        if (confirmResult && !confirmResult.success) {
          console.log(
            `   ⚠️  Payment confirmation function not available or failed: ${confirmResult.error}`,
          );
          return;
        }
    ...
    // ============================================
    await runner.runTest(
      "PAYMENT 5: Expired payments cannot be completed",
      async () => {
        ...
        const {
          paymentStateMachine,
        } = require("../lib/state/payment-state-machine");
