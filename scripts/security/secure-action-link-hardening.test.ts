...
export async function runSecureActionLinkHardeningTests(): Promise<void> {
  ...
  runTest("token_scope_accept_token_invalid_for_reject", () => {
    ...
    const validForReject = validateResourceToken(
      "reject",
      BOOKING_ID,
      acceptToken,
    );
    assert(
      validForReject === false,
      "accept token must not validate for reject action",
    );
  });
  ...
  runTest("token_scope_reject_token_invalid_for_accept", () => {
    ...
    const validForAccept = validateResourceToken(
      "accept",
      BOOKING_ID,
      rejectToken,
    );
    assert(
      validForAccept === false,
      "reject token must not validate for accept action",
    );
  });
