...
async function testRBACAuthorization() {
  ...
  try {
    ...
    await runner.runTest(
      "RBAC 1: Customer cannot modify businesses",
      async () => {
        ...
        console.log(
          `   ✅ Test completed (service role bypasses RLS - expected)`,
        );
        console.log(
          `   ⚠️  Note: API endpoints should enforce RBAC at application level`,
        );
      },
    );
    ...
    // ============================================
    await runner.runTest(
      "RBAC 2: Owner can only manage own business",
      async () => {
        const owner1 = await getOrCreateTestUser(
          `owner1-${Date.now()}@test.com`,
          "owner",
        );
        const owner2 = await getOrCreateTestUser(
        ...
        console.log(
          `   ⚠️  Note: API endpoints should enforce owner checks at application level`,
        );
      },
    );
    ...
    // ============================================
    await runner.runTest(
      "RBAC 3: Suspended business blocks bookings",
      async () => {
        const business = await getRandomBusiness();
    ...
    // ============================================
    await runner.runTest(
      "RBAC 4: Role verification works correctly",
      async () => {
        const customer = await getOrCreateTestUser(
          `customer-${Date.now()}@test.com`,
          "customer",
    ...
    // ============================================
    await runner.runTest(
      "RBAC 5: Missing profile handled correctly",
      async () => {
    ...
    // ============================================
    await runner.runTest(
      "RBAC 6: Admin can override restrictions",
      async () => {
        const business = await getRandomBusiness();
        ...
        console.log(
          `   ✅ Admin can access business data (override capability verified)`,
        );
      },
    );
  } finally {
