...
export function runUnitInputSanitizerTests(): void {
  ...
  runTest("should_sanitizeString_trim_and_remove_script_like_content", () => {
    ...
    assert(
      ...
      `Expected no angle brackets: '${out}'`,
    );
  });
  ...
  runTest("should_sanitizeNumber_return_number_when_valid_string", () => {
    assert(
      ...
      `Expected 42, got ${sanitizeNumber("42")}`,
    );
    assert(
      sanitizeNumber("3.14") === 3.14,
      `Expected 3.14, got ${sanitizeNumber("3.14")}`,
    );
  });
  ...
  runTest("should_sanitizeInteger_floor_decimal", () => {
    assert(
      sanitizeInteger(3.9) === 3,
      `Expected 3, got ${sanitizeInteger(3.9)}`,
    );
  });

  runTest(
    "should_sanitizeInteger_return_null_when_sanitizeNumber_returns_null",
    () => {
  ...
  runTest(
    "should_sanitizeDate_return_null_when_regex_valid_but_invalid_calendar",
    () => {
      assert(
        sanitizeDate("2025-13-01") === null,
        "Expected null for invalid month",
      );
    },
  );
  ...
  runTest("should_sanitizeTime_return_null_when_invalid", () => {
    assert(
      sanitizeTime("9:30") === null,
      "Expected null for missing leading zero",
    );
  });
  ...
  runTest("should_sanitizeObject_include_only_sanitized_values", () => {
    ...
    assert(
      (out as any).name === "John",
      `Expected trimmed name, got ${(out as any).name}`,
    );
  ...
  runTest(
    "should_sanitizeRequestBody_return_object_when_valid_json",
    async () => {
      const req = {
        json: async () => ({ foo: "bar" }),
      } as any;
  ...
  runTest(
    "should_sanitizeRequestBody_return_null_when_not_object",
    async () => {
