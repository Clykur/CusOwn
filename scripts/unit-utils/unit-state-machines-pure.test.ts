...
export function runUnitStateMachinesPureTests(): void {
  ...
  runTest('should_slot_canTransition_allow_reserved_to_booked_when_event_book', () => {
    assert(slotStateMachine.canTransition('reserved', 'book') === true, 'Expected true');
  });
  ...
  runTest('should_slot_canTransition_allow_reserved_to_available_on_expire', () => {
    assert(slotStateMachine.canTransition('reserved', 'expire') === true, 'Expected true');
  });

  runTest('should_slot_getNextState_return_available_when_reserved_expire', () => {
  ...
  runTest('should_booking_canTransition_allow_pending_to_cancelled_on_cancel', () => {
    assert(bookingStateMachine.canTransition('pending', 'cancel') === true, 'Expected true');
  });

  runTest('should_booking_canTransition_allow_pending_to_cancelled_on_expire', () => {
    assert(bookingStateMachine.canTransition('pending', 'expire') === true, 'Expected true');
  });

  runTest('should_booking_getNextState_return_cancelled_when_pending_cancel', () => {
    const next = bookingStateMachine.getNextState('pending', 'cancel');
    assert(next === 'cancelled', `Expected 'cancelled', got '${next}'`);
  });

  runTest('should_booking_getNextState_return_cancelled_when_pending_expire', () => {
    const next = bookingStateMachine.getNextState('pending', 'expire');
    assert(next === 'cancelled', `Expected 'cancelled', got '${next}'`);
  });

  runTest('should_booking_canTransition_allow_confirmed_to_cancelled_on_cancel', () => {
    assert(bookingStateMachine.canTransition('confirmed', 'cancel') === true, 'Expected true');
  });

  runTest('should_booking_getNextState_return_cancelled_when_confirmed_cancel', () => {
  ...
  runTest('should_payment_canTransition_allow_initiated_to_expired_on_expire', () => {
    assert(paymentStateMachine.canTransition('initiated', 'expire') === true, 'Expected true');
  });

  runTest('should_payment_getNextState_return_expired_when_initiated_expire', () => {
    const next = paymentStateMachine.getNextState('initiated', 'expire');
    assert(next === 'expired', `Expected 'expired', got '${next}'`);
  });

  runTest('should_booking_validateTransition_return_true_for_pending_cancel_to_cancelled', () => {
    assert(
      bookingStateMachine.validateTransition('pending', 'cancelled', 'cancel') === true,
      'Expected true'
    );
  });

  runTest('should_slot_validateTransition_return_true_for_reserved_book_to_booked', () => {
    assert(
      slotStateMachine.validateTransition('reserved', 'booked', 'book') === true,
      'Expected true'
    );
  });
}
