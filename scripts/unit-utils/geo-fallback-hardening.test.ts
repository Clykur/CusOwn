...
export async function runGeoFallbackHardeningTests(): Promise<void> {
  ...
  await runTest(
    "fallback metric for geo_provider is METRICS_DISCOVERY_FALLBACK_GEO",
    () => {
  ...
  await runTest(
    "fallback metric for rpc is METRICS_DISCOVERY_FALLBACK_RPC",
    () => {
  ...
  await runTest(
    "queryDiscoveryFallback returns empty array on error",
    async () => {
      const mockSupabase = {
        from: () => ({
          select: () => ({
  ...
  await runTest("metrics increment choice matches fallback reason", () => {
    assert(
      getFallbackMetricForReason("geo_provider") ===
        METRICS_DISCOVERY_FALLBACK_GEO,
