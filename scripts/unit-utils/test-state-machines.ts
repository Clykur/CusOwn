...
async function testStateMachines() {
  ...
  try {
    ...
    await runner.runTest(
      "Slot: available → reserved (valid transition)",
      async () => {
    ...
    await runner.runTest(
      "Slot: reserved → booked (valid transition)",
      async () => {
        const slot = (global as any).testSlot;
    ...
    await runner.runTest(
      "Slot: booked → available (invalid transition)",
      async () => {
        const slot = (global as any).testSlot;
        ...
        console.log(
          `   Correctly rejected invalid transition: booked → available`,
        );
      },
    );

    await runner.runTest(
      "Booking: pending → confirmed (valid transition)",
      async () => {
    ...
    await runner.runTest(
      "Booking: confirmed → pending (invalid transition)",
      async () => {
        const business = (global as any).testBusiness;
        const slot = await getRandomAvailableSlot(business.id);
        ...
        console.log(
          `   Correctly rejected invalid transition: confirmed → pending`,
        );
      },
    );

    await runner.runTest(
      "Payment: initiated → completed (valid transition)",
      async () => {
        const business = (global as any).testBusiness;
        const slot = await getRandomAvailableSlot(business.id);
        ...
        if (completeError) {
          throw new Error(
            `Failed to complete payment: ${completeError.message}`,
          );
        }
