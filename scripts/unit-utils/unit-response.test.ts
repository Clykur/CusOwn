...
export async function runUnitResponseTests(): Promise<void> {
  ...
  await runAsyncTest("successResponse includes optional message", async () => {
    ...
    assert(
      body.message === "Done",
      `Expected message Done, got ${body.message}`,
    );
  });

  await runAsyncTest(
    "errorResponse returns given status and body with success false",
    async () => {
      ...
      assert(
        ...
        `Expected error message, got ${body.error}`,
      );
    },
  );

  await runAsyncTest(
    "errorResponse defaults to 400 when status omitted",
    async () => {
