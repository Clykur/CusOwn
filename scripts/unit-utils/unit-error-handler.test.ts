...
export function runUnitErrorHandlerTests(): void {
  ...
  runTest(
    "getUserFriendlyError returns network message for fetch errors",
    () => {
  ...
  runTest(
    "getUserFriendlyError returns BOOKING_LINK_EXISTS for duplicate booking_link",
    () => {
      const out = getUserFriendlyError(
        new Error("unique constraint booking_link"),
      );
      assert(
  ...
  runTest(
    "getUserFriendlyError returns CREATE_BUSINESS_FAILED for generic duplicate key",
    () => {
  ...
  runTest(
    "getUserFriendlyError returns validation message as-is when required/invalid",
    () => {
  ...
  runTest(
    "getUserFriendlyError returns permission message for 403-like",
    () => {
      const out = getUserFriendlyError(new Error("unauthorized"));
      assert(
        out.toLowerCase().includes("permission") ||
  ...
  runTest(
    "getUserFriendlyError returns QR message for qr/qrcode errors",
    () => {
      const out = getUserFriendlyError(new Error("qr code generation failed"));
      assert(
        out.includes("QR") ||
          out.includes("qrcode") ||
          out.includes("dashboard"),
  ...
  runTest(
    "getUserFriendlyError returns short message as-is when user-friendly",
    () => {
...
export async function runUnitErrorHandlerAsyncTests(): Promise<void> {
  runAsyncTest("handleApiError returns message for 400 status", async () => {
    ...
    assert(
      out.toLowerCase().includes("invalid") ||
        out.toLowerCase().includes("request"),
  ...
  runAsyncTest("handleApiError returns message for 401 status", async () => {
    ...
    const out = await handleApiError(res);
    assert(
      out.toLowerCase().includes("authorized") ||
        out.toLowerCase().includes("auth"),
  ...
  runAsyncTest(
    "handleApiError returns 429 message for rate limit",
    async () => {
      const res = new Response(JSON.stringify({}), { status: 429 });
      const out = await handleApiError(res);
      assert(
        out.toLowerCase().includes("many") ||
          out.toLowerCase().includes("wait"),
  ...
  runAsyncTest("handleApiError returns message for 404 status", async () => {
    ...
    const out = await handleApiError(res);
    assert(
      out.toLowerCase().includes("not found"),
      `Expected 404 message, got '${out}'`,
    );
  });
  ...
  runAsyncTest("handleApiError returns message for 409 status", async () => {
    ...
    assert(
      out.toLowerCase().includes("conflict") ||
        out.toLowerCase().includes("exists"),
  ...
  runAsyncTest("handleApiError returns message for 503 status", async () => {
    ...
    const out = await handleApiError(res);
    assert(
      out.toLowerCase().includes("unavailable") ||
        out.toLowerCase().includes("try again"),
      `Expected 503 message, got '${out}'`,
    );
  });

  runAsyncTest(
    "handleApiError uses data.message when present and no data.error",
    async () => {
  ...
  runAsyncTest(
    "handleApiError returns generic message for unknown status",
    async () => {
