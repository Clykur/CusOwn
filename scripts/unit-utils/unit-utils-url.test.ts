...
function createMockRequest(
  ...
  return {
    ...
    headers: {
      get: (name: string) => {
        if (name === "host")
          return host !== undefined ? host : new URL(origin).host;
...
export function runUnitUtilsUrlTests(): void {
  ...
  runTest("should_getBaseUrl_return_origin_when_request_has_origin", () => {
    ...
    assert(
      ...
      `Expected custom origin, got ${base}`,
    );
  });

  runTest(
    "should_getBaseUrl_use_host_and_protocol_when_origin_localhost",
    () => {
      const req = {
      ...
      const base = getBaseUrl(req);
      assert(
        base === "https://myhost:3000",
        `Expected https://myhost:3000, got ${base}`,
      );
    },
  );

  runTest(
    "should_getBaseUrl_use_http_when_host_is_localhost_and_no_x_forwarded_proto",
    () => {
      ...
      assert(
        ...
        `Expected http://localhost:3000, got ${base}`,
      );
    },
  );

  runTest(
    "should_getBaseUrl_use_https_when_host_non_localhost_no_x_forwarded_proto",
    () => {
      const req = createMockRequest(
        "http://127.0.0.1:3000",
        "app.example.com:443",
      ...
      const base = getBaseUrl(req);
      assert(
        base === "https://app.example.com:443",
        `Expected https, got ${base}`,
      );
    },
  );

  runTest(
    "should_getBaseUrl_fall_through_to_node_when_request_has_no_host",
    () => {
      const req = {
        nextUrl: { origin: "http://localhost:3000" },
        headers: { get: () => null },
  ...
  runTest(
    "should_getBaseUrl_return_VERCEL_URL_https_when_set_and_no_app_url",
    () => {
      const prev = process.env.NEXT_PUBLIC_APP_URL;
      const prevVercel = process.env.VERCEL_URL;
      try {
        ...
        const base = getBaseUrl();
        assert(
          base === "https://myproject.vercel.app",
          `Expected vercel URL, got ${base}`,
        );
      } finally {
        process.env.NEXT_PUBLIC_APP_URL = prev;
        process.env.VERCEL_URL = prevVercel;
  ...
  runTest(
    "should_getBaseUrl_return_production_fallback_when_NODE_ENV_production",
    () => {
      ...
      try {
        ...
        const base = getBaseUrl();
        assert(
          base === "https://cusown.clykur.com",
          `Expected production URL, got ${base}`,
        );
      } finally {
  ...
  runTest(
    "should_getBaseUrl_return_localhost_fallback_when_NEXT_PUBLIC_APP_URL_unset",
    () => {
      ...
      try {
        ...
        assert(
          base === "http://localhost:3000",
          `Expected localhost fallback, got ${base}`,
        );
      } finally {
        process.env.NODE_ENV = prevNode;
        process.env.NEXT_PUBLIC_APP_URL = prevApp;
  ...
  runTest(
    "should_getBaseUrl_final_return_ternary_true_when_browser_path",
    () => {
      const prevNode = (process.versions as any).node;
      const prevWindow = (globalThis as any).window;
  ...
  runTest(
    "should_getBaseUrl_final_return_ternary_false_when_browser_path",
    () => {
      const prevNode = (process.versions as any).node;
      const prevWindow = (globalThis as any).window;
      const prevNodeEnv = process.env.NODE_ENV;
      ...
      try {
        ...
        assert(
          base === "http://localhost:3000" ||
            (base.length > 0 && base.startsWith("http")),
  ...
  runTest(
    "should_getBaseUrl_return_window_origin_when_browser_and_no_request",
    () => {
      const prevNode = (process.versions as any).node;
      const prevWindow = (globalThis as any).window;
      try {
        ...
        const base = getBaseUrl();
        assert(
          base === "https://browser-origin.test",
          `Expected browser origin, got ${base}`,
        );
      } finally {
        (process.versions as any).node = prevNode;
        (globalThis as any).window = prevWindow;
  ...
  runTest("should_getBookingUrl_include_base_and_prefix_and_link", () => {
    ...
    assert(
      url === "https://app.com/book/my-link",
      `Expected booking URL, got ${url}`,
    );
  });
  ...
  runTest("should_getBookingStatusUrl_include_booking_id", () => {
    ...
    assert(
      url === "https://app.com/booking/booking-id-123",
      `Expected status URL, got ${url}`,
    );
  });
  ...
  runTest("should_getClientBaseUrl_return_NEXT_PUBLIC_APP_URL_when_set", () => {
    ...
    try {
      ...
      assert(
        base === "https://client.test",
        `Expected client.test, got ${base}`,
      );
    } finally {
      process.env.NEXT_PUBLIC_APP_URL = prev;
      process.env.VERCEL_URL = prevVercel;
  ...
  runTest(
    "should_getClientBaseUrl_return_VERCEL_URL_when_set_and_no_app_url",
    () => {
      const prev = process.env.NEXT_PUBLIC_APP_URL;
      const prevVercel = process.env.VERCEL_URL;
      try {
        ...
        const base = getClientBaseUrl();
        assert(
          base === "https://client.vercel.app",
          `Expected vercel, got ${base}`,
        );
      } finally {
        process.env.NEXT_PUBLIC_APP_URL = prev;
        process.env.VERCEL_URL = prevVercel;
  ...
  runTest(
    "should_getClientBaseUrl_return_production_when_NODE_ENV_production",
    () => {
      const prevNode = process.env.NODE_ENV;
      const prevApp = process.env.NEXT_PUBLIC_APP_URL;
      const prevVercel = process.env.VERCEL_URL;
  ...
  runTest(
    "should_getClientBaseUrl_fallback_appUrl_then_vercel_then_return",
    () => {
      const prevNode = (process.versions as any).node;
      const prevApp = process.env.NEXT_PUBLIC_APP_URL;
      const prevVercel = process.env.VERCEL_URL;
      ...
      try {
        ...
        const base = getClientBaseUrl();
        assert(
          base === "https://fallback-app.test",
          `Expected fallback app, got ${base}`,
        );
      } finally {
        (process.versions as any).node = prevNode;
        process.env.NEXT_PUBLIC_APP_URL = prevApp;
  ...
  runTest("should_getClientBaseUrl_fallback_vercel_when_no_app_url", () => {
    ...
    try {
      ...
      const base = getClientBaseUrl();
      assert(
        base === "https://fallback.vercel.app",
        `Expected fallback vercel, got ${base}`,
      );
    } finally {
      (process.versions as any).node = prevNode;
      process.env.NEXT_PUBLIC_APP_URL = prevApp;
  ...
  runTest(
    "should_getClientBaseUrl_return_env_baseUrl_when_appUrl_localhost",
    () => {
      const prevNode = process.env.NODE_ENV;
      const prevApp = process.env.NEXT_PUBLIC_APP_URL;
      const prevVercel = process.env.VERCEL_URL;
  ...
  runTest("should_getClientBaseUrl_return_window_origin_when_browser", () => {
    ...
    try {
      ...
      const base = getClientBaseUrl();
      assert(
        base === "https://client-browser.test",
        `Expected client browser origin, got ${base}`,
      );
    } finally {
      (process.versions as any).node = prevNode;
      (globalThis as any).window = prevWindow;
  ...
  runTest("should_getBaseUrl_without_request_return_string_in_node", () => {
    ...
    assert(
      typeof base === "string" && base.length > 0,
      `Expected non-empty string, got ${base}`,
    );
  });
}
