...
export async function runUnitRetryTests(): Promise<void> {
  ...
  await runAsyncTest(
    "retry throws immediately when error is not retryable",
    async () => {
      let attempts = 0;
      try {
        await retry(async () => {
        ...
        assert(
          (e as Error).message === "validation error",
          "Expected same error",
        );
      }
    },
  ...
  await runAsyncTest(
    "retry eventually succeeds after retryable failures",
    async () => {
      let attempts = 0;
  ...
  await runAsyncTest(
    "retry treats network and econnreset as retryable by default",
    async () => {
      let attempts = 0;
      const result = await retry(
        async () => {
  ...
  await runAsyncTest(
    "retry throws last error after all attempts exhausted",
    async () => {
  ...
  await runAsyncTest(
    "retry does not retry when retryable returns false",
    async () => {
      let attempts = 0;
      try {
        await retry(
