...
export function runUnitUtilsSecurityTests(): void {
  ...
  runTest("should_isValidUUID_accept_lowercase_hex", () => {
    assert(
      isValidUUID("a1b2c3d4-e5f6-4789-a012-3456789abcde") === true,
      "Expected true",
    );
  });
  ...
  runTest("should_sanitizeInput_trim_whitespace_when_present", () => {
    assert(
      sanitizeInput("  foo  ") === "foo",
      `Expected 'foo', got '${sanitizeInput("  foo  ")}'`,
    );
  });
  ...
  runTest("should_sanitizeInput_remove_angle_brackets_when_present", () => {
    ...
    assert(
      sanitizeInput("a>b<c") === "abc",
      `Expected 'abc', got '${sanitizeInput("a>b<c")}'`,
    );
  });
  ...
  runTest("should_generateResourceToken_return_64_char_hex_when_called", () => {
    ...
    assert(
      ...
      `Expected 64-char hex, got length ${token.length}`,
    );
  });

  runTest("should_validateResourceToken_return_false_when_token_empty", () => {
    assert(
      validateResourceToken("salon", "id", "") === false,
      "Expected false",
    );
  });

  runTest(
    "should_validateResourceToken_return_false_when_resourceId_empty",
    () => {
      assert(
        validateResourceToken("salon", "", "a".repeat(64)) === false,
        "Expected false",
      );
    },
  );
  ...
  runTest(
    "should_validateResourceToken_return_false_when_token_wrong_length",
    () => {
      assert(
        validateResourceToken("salon", "id", "abc") === false,
        "Expected false for short token",
      );
    },
  );
  ...
  runTest(
    "should_getSecureResourceUrl_return_url_with_token_when_baseUrl_provided",
    () => {
      const url = getSecureResourceUrl(
        "salon",
        "sid-123",
        "https://example.com",
      );
      ...
      const acceptUrl = getSecureResourceUrl(
        "accept",
        "bid-456",
        "https://app.com",
      );
      assert(
        acceptUrl.includes("/accept/bid-456?token="),
        `Expected accept URL, got ${acceptUrl}`,
      );
      const rejectUrl = getSecureResourceUrl(
        "reject",
        "bid-789",
        "https://app.com",
      );
      assert(
        rejectUrl.includes("/reject/bid-789?token="),
        `Expected reject URL`,
      );
      const ownerUrl = getSecureResourceUrl(
        "owner-dashboard",
        "link-1",
        "https://app.com",
      );
      assert(ownerUrl.includes("/owner/link-1?token="), `Expected owner URL`);
      const adminBizUrl = getSecureResourceUrl(
        "admin-business",
        "biz-1",
        "https://app.com",
      );
      assert(
        adminBizUrl.includes("/admin/businesses/biz-1?token="),
        `Expected admin-business URL`,
      );
      const adminBookUrl = getSecureResourceUrl(
        "admin-booking",
        "book-1",
        "https://app.com",
      );
      assert(
        adminBookUrl.includes("/admin/bookings/book-1?token="),
        `Expected admin-booking URL`,
      );
      const bookingUrl = getSecureResourceUrl(
        "booking",
        "link-x",
        "https://app.com",
      );
      assert(bookingUrl.includes("/b/link-x?token="), `Expected booking URL`);
      const statusUrl = getSecureResourceUrl(
        "booking-status",
        "bid-y",
        "https://app.com",
      );
      assert(
        statusUrl.includes("/booking/bid-y?token="),
        `Expected booking-status URL`,
      );
    },
  );
  ...
  runTest("should_getSecureSalonUrl_return_salon_url_with_base", () => {
    ...
    assert(
      url.startsWith("https://base.com/salon/my-salon?token="),
      `Got ${url}`,
    );
  });

  runTest(
    "should_getSecureResourceUrl_use_getBaseUrl_when_baseUrl_omitted",
    () => {
  ...
  runTest(
    "should_validateBookingAccess_return_true_when_no_requestedSalonId",
    () => {
  ...
  runTest("should_validateBookingAccess_return_true_when_match", () => {
    assert(
      validateBookingAccess("salon-a", "salon-a") === true,
      "Expected true",
    );
  });
  ...
  runTest("should_validateBookingAccess_return_false_when_mismatch", () => {
    assert(
      validateBookingAccess("salon-a", "salon-b") === false,
      "Expected false",
    );
  });

  runTest(
    "should_validateResourceToken_return_true_when_legacy_32_char_token_valid",
    () => {
  ...
  runTest(
    "should_validateResourceToken_return_true_when_legacy_16_char_token_valid",
    () => {
      const secret = process.env.SALON_TOKEN_SECRET;
      if (!secret) return;
  ...
  runTest(
    "should_validateSalonToken_return_true_when_legacy_token_valid",
    () => {
      const secret = process.env.SALON_TOKEN_SECRET;
      if (!secret) return;
  ...
  runTest(
    "should_validateResourceToken_return_false_when_token_expired",
    () => {
      const secret = process.env.SALON_TOKEN_SECRET;
      if (!secret) return;
      ...
      assert(
        validateResourceToken("salon", resourceId, token, currentTime) ===
          false,
