...
export function runUnitReviewsTests(): void {
  ...
  runTest("containsProfanity returns true for blocked word", () => {
    ...
    assert(
      containsProfanity("Some text blockedword here"),
      "substring should be blocked",
    );
  ...
  runTest(
    "duplicate prevention: error message matches constant for 409 mapping",
    () => {
  ...
  runTest("aggregate correctness: profanity list is defined for filter", () => {
    ...
    assert(
      REVIEW_PROFANITY_WORDS.length >= 1,
      "at least one test word for filter",
    );
  });
}
