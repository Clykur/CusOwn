...
export async function runMetricsReliabilityHardeningTests(): Promise<void> {
  ...
  await runTest(
    "safeIncrement never throws when underlying increment throws",
    async () => {
  ...
  await runTest(
    ...
    async () => {
      ...
      try {
        ...
        assert(
          parsed.message === "Metrics operation failed",
          "Expected failure message",
        );
        assert(
          parsed.service === METRICS_SERVICE_NAME,
          "Expected service name",
        );
        assert(
          parsed.metric_name === "api.booking.created",
  ...
  await runTest(
    "safeRecordTiming never throws when underlying recordTiming throws",
    async () => {
      const impl: MetricsLike = {
        increment: async () => {},
        recordTiming: async () => {
  ...
  await runTest(
    "safeSetGauge never throws when underlying setGauge throws",
    async () => {
      const impl: MetricsLike = {
        increment: async () => {},
        recordTiming: async () => {},
