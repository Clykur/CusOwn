...
export function runUnitBusinessDiscoveryRankingTests(): void {
  ...
  runTest(
    "sortByRankingScore orders by score DESC then business_id ASC",
    () => {
      const items = [
      ...
      assert(
        sorted[0].score === 1 && sorted[0].business_id === "a",
        "First: score 1, id a",
      );
      assert(
        sorted[1].score === 1 && sorted[1].business_id === "b",
        "Second: score 1, id b",
      );
      assert(
        sorted[2].score === 0.5 && sorted[2].business_id === "c",
        "Third: score 0.5, id c",
      );
    },
  );
  ...
  runTest("sortByRankingScore is deterministic for same input", () => {
    ...
    assert(
      first.every(
        (x, i) =>
          x.business_id === second[i].business_id &&
          x.score === second[i].score,
      ),
