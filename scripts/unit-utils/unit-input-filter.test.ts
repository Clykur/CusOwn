...
export function runUnitInputFilterTests(): void {
  ...
  runTest(
    "should_filterFields_include_only_allowed_keys_when_extra_keys_present",
    () => {
  ...
  runTest(
    "should_filterBookingUpdateFields_allow_status_customer_name_phone",
    () => {
      const body = {
        status: "confirmed",
        customer_name: "Jane",
  ...
  runTest(
    "should_filterBusinessUpdateFields_allow_salon_name_slot_duration",
    () => {
  ...
  runTest(
    "should_filterUserProfileUpdateFields_allow_user_type_full_name",
    () => {
      const body = {
        user_type: "owner",
        full_name: "John",
  ...
  runTest("should_validateStringLength_return_true_when_within_max", () => {
    ...
    assert(
      validateStringLength("hello", 5) === true,
      "Expected true at boundary",
    );
  });
  ...
  runTest(
    "should_filterOwnerBusinessUpdateFields_allow_capacity_and_structured_address",
    () => {
      const body = {
        salon_name: "X",
