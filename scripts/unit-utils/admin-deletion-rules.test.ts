...
export function runAdminDeletionRulesTests(): void {
  ...
  runTest("deletion with valid reason succeeds", () => {
    const err = validateAdminDeletionReason(
      "Compliance: user requested account closure",
    );
    assert(err === null, `Expected valid, got ${err}`);
  });

  runTest(
    "deletion blocked when dependencies exist (outstanding payments)",
    () => {
  ...
  runTest("deletion blocked when dependencies exist (last admin)", () => {
    assert(
      isDependencyBlockError("Cannot delete the last admin"),
  ...
  runTest("deletion blocked when dependencies exist (active bookings)", () => {
    const msg =
      "Cannot soft-delete business: active (pending or confirmed) bookings exist";
    assert(isDependencyBlockError(msg), "Expected dependency block");
  });

  runTest(
    "deletion blocked when dependencies exist (already soft-deleted)",
    () => {
      assert(
  ...
  runTest("non-dependency error is not classified as dependency block", () => {
    assert(
      !isDependencyBlockError("User not found"),
      "Expected not dependency block",
    );
    assert(
      !isDependencyBlockError("Database error"),
      "Expected not dependency block",
    );
  });
  ...
  runTest("reason required error is detected", () => {
    assert(
      isReasonRequiredError("Deletion reason is mandatory"),
  ...
  runTest(
    "deleting an already-deleted business returns safe error message",
    () => {
      assert(
        typeof ERROR_MESSAGES.BUSINESS_ALREADY_DELETED === "string" &&
          ERROR_MESSAGES.BUSINESS_ALREADY_DELETED.length > 0 &&
          !ERROR_MESSAGES.BUSINESS_ALREADY_DELETED.toLowerCase().includes(
            "table",
          ) &&
          !ERROR_MESSAGES.BUSINESS_ALREADY_DELETED.toLowerCase().includes(
            "column",
  ...
  runTest(
    "audit dedupe window prevents duplicate logs for same deletion",
    () => {
      assert(
