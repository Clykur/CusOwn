...
import {
  withRequestContext,
  getCorrelationId,
} from "../../lib/monitoring/request-context";
...
export async function runUnitRequestContextTests(): Promise<void> {
  ...
  runAsyncTest(
    "getCorrelationId returns undefined when not in context",
    async () => {
  ...
  runAsyncTest(
    "withRequestContext runs handler and returns result",
    async () => {
      ...
      const result = await withRequestContext(req, async (ctx) => {
        assert(
          ctx.correlationId === "test-correlation-123",
          "Expected passed correlation id",
        );
        return 42;
      });
  ...
  runAsyncTest(
    "getCorrelationId returns id inside withRequestContext",
    async () => {
      const req = new Request("http://localhost/", {
  ...
  runAsyncTest(
    "withRequestContext generates new UUID when no header",
    async () => {
      const req = new Request("http://localhost/");
      await withRequestContext(req, async (ctx) => {
        ...
        assert(
          /^[0-9a-f-]+$/i.test(ctx.correlationId),
          "Expected UUID-like format",
        );
      });
    },
  );
