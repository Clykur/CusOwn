...
export function runUnitMagicBytesTests(): void {
  ...
  runTest(
    "resolveContentTypeFromMagicBytes returns null for empty buffer",
    () => {
  ...
  runTest(
    "resolveContentTypeFromMagicBytes returns null for buffer shorter than 12",
    () => {
  ...
  runTest(
    "resolveContentTypeFromMagicBytes returns image/jpeg for JPEG signature",
    () => {
  ...
  runTest(
    "resolveContentTypeFromMagicBytes returns image/png for PNG signature",
    () => {
  ...
  runTest(
    "validateMagicBytes returns valid false and error for disallowed declared type",
    () => {
  ...
  runTest(
    "validateMagicBytes returns valid true when declared matches signature",
    () => {
      const buf = Buffer.alloc(20);
      buf[0] = 0xff;
      buf[1] = 0xd8;
  ...
  runTest(
    "validateMagicBytes returns valid false when declared does not match signature",
    () => {
      const buf = Buffer.from([
        0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, 0, 0, 0, 0,
      ]);
      ...
      assert(
        out.resolvedMime === "image/png",
        "Expected resolvedMime to be png",
      );
    },
  );
}
