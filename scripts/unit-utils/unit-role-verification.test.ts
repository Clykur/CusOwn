...
export function runUnitRoleVerificationTests(): void {
  ...
  runTest("isAdminProfile returns true when user_type is admin", () => {
    assert(
      isAdminProfile({ user_type: "admin" }) === true,
  ...
  runTest("isAdminProfile returns false when user_type is owner", () => {
    assert(
      isAdminProfile({ user_type: "owner" }) === false,
      "Expected false for owner",
    );
  });
...
export async function runUnitRoleVerificationAsyncTests(): Promise<void> {
  runAsyncTest(
    "hasOwnerAccess returns true when profile is owner and passed",
    async () => {
  ...
  runAsyncTest(
    "hasOwnerAccess returns false when profile is customer and passed",
    async () => {
  ...
  runAsyncTest(
    "hasCustomerAccess returns true when profile is customer and passed",
    async () => {
  ...
  runAsyncTest(
    "hasAdminAccess returns true when profile is admin and passed",
    async () => {
  ...
  runAsyncTest(
    "hasAdminAccess returns false when profile is null and passed",
    async () => {
