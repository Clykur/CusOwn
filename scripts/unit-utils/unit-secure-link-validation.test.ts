...
export async function runUnitSecureLinkValidationTests(): Promise<void> {
  ...
  await runAsyncTest(
    ...
    async () => {
      const result = await validateOwnerActionLink(
        "salon" as "accept",
        "bid",
        "token",
      );
      ...
      assert(
        (result as { reason: string }).reason === "invalid",
  ...
  await runAsyncTest(
    "validateOwnerActionLink returns invalid for empty token",
    async () => {
  ...
  await runAsyncTest(
    "validateOwnerActionLink returns invalid for empty resourceId",
    async () => {
