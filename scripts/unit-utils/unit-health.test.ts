...
export async function runUnitHealthTests(): Promise<void> {
  ...
  await runAsyncTest(
    "checkMediaHealth returns status and media checks",
    async () => {
      ...
      assert(
        result.checks.media_storage === "up" ||
          result.checks.media_storage === "down",
        "media_storage",
      );
      assert(
        result.checks.media_table === "up" ||
          result.checks.media_table === "down",
        "media_table",
      );
