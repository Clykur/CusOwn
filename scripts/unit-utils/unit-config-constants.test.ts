...
export function runUnitConfigConstantsTests(): void {
  ...
  runTest(
    "should_WHATSAPP_MESSAGE_TEMPLATES_BOOKING_REQUEST_return_string",
    () => {
  ...
  runTest(
    "should_WHATSAPP_MESSAGE_TEMPLATES_CONFIRMATION_return_string",
    () => {
      // pragma: allowlist secret
      ...
      assert(typeof msg === "string", "Expected string");
      assert(
        msg.includes("Salon Name") &&
          msg.includes("123 Street") &&
          msg.includes("maps.link"),
        "Expected all params",
      );
    },
  );
  ...
  runTest("should_WHATSAPP_MESSAGE_TEMPLATES_REJECTION_return_string", () => {
    // pragma: allowlist secret
    const msg = WHATSAPP_MESSAGE_TEMPLATES.REJECTION(
      "Customer",
      "https://book.link",
    );
    assert(typeof msg === "string", "Expected string");
    assert(
      msg.includes("Customer") && msg.includes("book.link"),
      "Expected params",
    );
  });
  ...
  runTest("should_constants_export_values", () => {
    ...
    assert(
      typeof ERROR_MESSAGES.SALON_NAME_REQUIRED === "string",
      "ERROR_MESSAGES",
    );
