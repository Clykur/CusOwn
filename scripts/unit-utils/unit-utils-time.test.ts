...
export function runUnitUtilsTimeTests(): void {
  ...
  runTest("should_timeToMinutes_return_minutes_when_given_HH_MM", () => {
    assert(
      timeToMinutes("09:30") === 9 * 60 + 30,
      `Expected 570, got ${timeToMinutes("09:30")}`,
    );
    assert(
      timeToMinutes("00:00") === 0,
      `Expected 0, got ${timeToMinutes("00:00")}`,
    );
  });
  ...
  runTest("should_minutesToTime_return_HH_MM_SS_when_given_minutes", () => {
    assert(
      minutesToTime(570) === "09:30:00",
      `Expected 09:30:00, got ${minutesToTime(570)}`,
    );
    assert(
      minutesToTime(0) === "00:00:00",
      `Expected 00:00:00, got ${minutesToTime(0)}`,
    );
  });
  ...
  runTest(
    "should_isTimeBefore_return_true_when_first_earlier_than_second",
    () => {
  ...
  runTest(
    "should_generateTimeSlots_return_slots_when_valid_range_and_duration",
    () => {
  ...
  runTest(
    "should_generateTimeSlots_return_empty_when_closing_before_opening",
    () => {
  ...
  runTest("should_isTimeInRange_return_true_when_time_inside_range", () => {
    ...
    assert(
      isTimeInRange("08:00", "09:00", "12:00") === false,
      "Expected false",
    );
    assert(
      isTimeInRange("12:00", "09:00", "12:00") === false,
      "Expected false (exclusive end)",
    );
  });

  runTest(
    "should_isSlotTimePassed_return_false_when_slot_date_not_today",
    () => {
      const dateStr = "2030-01-15";
      assert(
        isSlotTimePassed(dateStr, "09:00:00") === false,
  ...
  runTest("should_isSlotInPast_return_true_when_slot_date_before_today", () => {
    ...
    assert(
      isSlotInPast(dateStr, "23:59:00") === true,
      "Past date should be in past",
    );
  });
  ...
  runTest("should_isSlotInPast_return_false_when_slot_date_after_today", () => {
    ...
    assert(
      isSlotInPast(dateStr, "00:00:00") === false,
      "Future date should not be in past",
    );
  });
  ...
  runTest("should_generateTimeSlots_handle_HH_MM_format_add_seconds", () => {
    ...
    assert(
      slots.length === 2,
      `Expected 2 slots for 09:00-11:00 60min, got ${slots.length}`,
    );
    assert(
      slots[0].start === "09:00:00" && slots[0].end === "10:00:00",
      `First slot mismatch`,
  ...
  runTest("should_isTimeInRange_return_false_when_at_closing", () => {
    assert(
      isTimeInRange("12:00", "09:00", "12:00") === false,
      "At closing is exclusive",
    );
  });

  runTest(
    "should_generateTimeSlots_handle_invalid_time_format_single_part",
    () => {
  ...
  runTest(
    "should_generateTimeSlots_normalizeTime_parts_length_3_return_unchanged",
    () => {
      const slots = generateTimeSlots("09:00:00", "10:00:00", 60);
      assert(
        slots.length === 1 &&
          slots[0].start === "09:00:00" &&
          slots[0].end === "10:00:00",
  ...
  runTest(
    "should_generateTimeSlots_normalizeTime_parts_length_2_add_seconds",
    () => {
  ...
  runTest(
    "should_generateTimeSlots_return_empty_when_opening_equals_closing",
    () => {
  ...
  runTest(
    "should_generateTimeSlots_safety_break_when_currentTime_ge_closing",
    () => {
      const slots = generateTimeSlots("09:00:00", "10:00:00", 60);
  ...
  runTest(
    "should_isSlotTimePassed_execute_today_branch_and_return_boolean",
    () => {
      const now = new Date();
  ...
  runTest(
    "should_isSlotTimePassed_execute_today_branch_with_future_time",
    () => {
      const now = new Date();
      const todayStr = now.toISOString().split("T")[0];
  ...
  runTest(
    "should_isSlotTimePassed_today_branch_return_true_when_slot_time_past",
    () => {
      const now = new Date();
      ...
      if (slotDateForToday !== null) {
        ...
        assert(
          result === true,
          `Expected true when slot today and midnight, got ${result}`,
        );
      }
    },
  );
  ...
  runTest(
    "should_isSlotTimePassed_today_branch_return_false_when_slot_time_future",
    () => {
      const now = new Date();
      const utcDate = now.toISOString().split("T")[0];
      const localStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-${String(now.getDate()).padStart(2, "0")}`;
      ...
      if (slotDateForToday !== null) {
        const future = new Date(now.getTime() + 2 * 60 * 60 * 1000);
        // Skip assertion if adding 2 hours crosses midnight: future.getHours() would
        // represent an earlier time of the same day, making isSlotTimePassed return true correctly.
        if (future.getDate() === now.getDate()) {
          const futureTime = `${String(future.getHours()).padStart(2, "0")}:${String(future.getMinutes()).padStart(2, "0")}:00`;
          const result = isSlotTimePassed(slotDateForToday, futureTime);
          assert(
            result === false,
            `Expected false when slot today and future time, got ${result}`,
          );
        }
      }
    },
  ...
  runTest("should_isSlotInPast_return_true_when_slot_date_before_today", () => {
    assert(
      isSlotInPast("2000-01-01", "23:59:00") === true,
      "Past date in past",
    );
  });
  ...
  runTest("should_isSlotInPast_return_false_when_slot_date_after_today", () => {
    assert(
      isSlotInPast("2030-12-31", "00:00:00") === false,
  ...
  runTest(
    "should_isSlotInPast_today_branch_return_true_when_slot_time_past",
    () => {
      const now = new Date();
      const utcDate = now.toISOString().split("T")[0];
      const localStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-${String(now.getDate()).padStart(2, "0")}`;
      ...
      if (slotDateForToday !== null) {
        ...
        assert(
          result === true,
          `Expected true when today and midnight, got ${result}`,
        );
      }
    },
  );
  ...
  runTest(
    "should_isSlotInPast_today_branch_return_false_when_slot_time_future",
    () => {
      const now = new Date();
      const utcDate = now.toISOString().split("T")[0];
      const localStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-${String(now.getDate()).padStart(2, "0")}`;
      ...
      if (slotDateForToday !== null) {
        ...
        if (future.getDate() === now.getDate()) {
          ...
          assert(
            result === false,
            `Expected false when today and future time, got ${result}`,
          );
        }
      }
    },
  ...
  runTest("should_timeToMinutes_handle_single_part", () => {
    assert(
      timeToMinutes("09") === 9 * 60,
      `Expected 540, got ${timeToMinutes("09")}`,
    );
  });

  runTest(
    "should_generateTimeSlots_return_empty_when_slotDuration_negative",
    () => {
