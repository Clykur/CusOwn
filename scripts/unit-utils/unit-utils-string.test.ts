...
export function runUnitUtilsStringTests(): void {
  ...
  runTest(
    "should_slug_lowercase_and_replace_spaces_when_given_mixed_case",
    () => {
      const out = generateSlug("Hello World Test");
      assert(
        out === "hello-world-test",
        `Expected 'hello-world-test', got '${out}'`,
      );
    },
  );
  ...
  runTest(
    "should_slug_replace_special_chars_with_empty_when_given_punctuation",
    () => {
  ...
  runTest("should_generateUniqueId_return_7_char_uppercase_when_called", () => {
    ...
    assert(
      /^[A-Z0-9]+$/.test(id),
      `Expected alphanumeric uppercase, got '${id}'`,
    );
  });
  ...
  runTest("should_formatTime_return_HH_MM_when_given_HH_MM_SS", () => {
    assert(
      ...
      `Expected '09:30', got '${formatTime("09:30:00")}'`,
    );
    assert(
      formatTime("14:00:00") === "14:00",
      `Expected '14:00', got '${formatTime("14:00:00")}'`,
    );
  });
  ...
  runTest(
    "should_formatPhoneNumber_leave_unchanged_when_already_plus91",
    () => {
      assert(
  ...
  runTest(
    "should_formatPhoneNumber_strip_spaces_and_dashes_when_present",
    () => {
  ...
  runTest(
    "should_formatPhoneNumber_handle_91_prefix_without_plus_when_12_digits",
    () => {
  ...
  runTest(
    "should_formatPhoneNumber_return_with_plus_when_already_has_plus_prefix",
    () => {
  ...
  runTest(
    "should_formatPhoneNumber_add_plus91_take_last_10_when_10_to_12_digits",
    () => {
      const out = formatPhoneNumber("919876543210");
      assert(out === "+919876543210", `Expected +919876543210, got '${out}'`);
      ...
      assert(
        out2 === "+913456789012",
        `Expected +91 + last 10 digits, got '${out2}'`,
      );
    },
  );
