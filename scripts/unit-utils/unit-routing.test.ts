...
import {
  WeightedGraph,
  GraphNode,
} from "../../lib/routing/graph-data-structures";
import {
  dijkstra,
  aStar,
  reconstructPath,
...
} from "../../lib/routing/road-network-loader";
...
async function runTests() {
  ...
  assertEquals(
    graph.getEdgeCount(),
    8,
    "Graph has 8 edges (4 bidirectional = 8 directed)",
  );
  ...
  assertEquals(
    pathAE,
    ["A", "B", "C", "E"],
    "Dijkstra reconstructs correct path A->E",
  );
  ...
  assertEquals(
    aStarResult.distances.get("E"),
    3.3,
    "A* finds correct distance A->E",
  );
  ...
  assertEquals(
    pathAEAstar,
    ["A", "B", "C", "E"],
    "A* reconstructs correct path A->E",
  );
  ...
  const route_not_null = assertNotNull(
    route,
    "computeRoute returns a route for A->E",
  );
  if (route_not_null) {
    assertEquals(
      route_not_null.totalDistanceKm,
      3.3,
      "Route total distance is 3.3 km",
    );
    assertInRange(
  ...
  assertEquals(
    withinRadius.length,
    4,
    "KD-tree finds all nodes within large radius",
  );
  ...
  assertEquals(
    service.isReady(),
    true,
    "Routing service is ready after initialization",
  );
  ...
  assertEquals(
    routeResult.routed,
    true,
    "Routing service computes routed result",
  );
  assertInRange(
    routeResult.distance_km,
    3.0,
    3.5,
    "Routing service computes reasonable distance",
  );
  assertInRange(
  ...
  assertEquals(
    cacheStatsAfter.cacheSize,
    1,
    "Cache has 1 entry after computing route",
  );
  ...
  assertEquals(
    warned,
    true,
    "Deviation warning emitted when OSRM differs from internal",
  );
  ...
  assert(
    cacheStatsPost.cacheSize >= 1,
    "Cache still present after second call",
  );
  ...
  assertEquals(
    fallbackRoute.mode,
    "walking",
    "Fallback route has correct mode",
  );
  assertInRange(
  ...
  try {
    ...
    osmErrorCaught = true;
    console.log(
      "✅ PASS: loadGraphFromOsmPbf throws when parser missing or file invalid",
    );
  }
  ...
  assertEquals(
    nodeCountAfter > nodeCountBefore,
    true,
    "Business nodes are added to graph",
  );
  ...
  console.log(
    `  Avg compute time: ${cacheStats.avgComputeTimeMs.toFixed(2)}ms`,
  );
