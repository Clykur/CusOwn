...
export async function runUnitCircuitBreakerTests(): Promise<void> {
  ...
  await runAsyncTest(
    "execute opens circuit after failureThreshold failures",
    async () => {
      const cb = new CircuitBreaker({
        failureThreshold: 2,
      ...
      try {
        ...
        assert(
          (e as Error).message === "Circuit breaker is OPEN",
          "Expected OPEN error",
        );
      }
    },
  ...
  await runAsyncTest(
    "half-open allows one call after resetTimeoutMs",
    async () => {
      const cb = new CircuitBreaker({
        failureThreshold: 1,
        resetTimeoutMs: 50,
      ...
      assert(
        cb.getState() === CircuitState.CLOSED,
  ...
  await runAsyncTest(
    "half-open allows only halfOpenMaxCalls then reopens on failure",
    async () => {
      const cb = new CircuitBreaker({
        failureThreshold: 1,
        resetTimeoutMs: 50,
