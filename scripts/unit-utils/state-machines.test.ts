...
async function testStateMachines() {
  ...
  try {
    ...
    await runner.runTest(
      "STATE 1: Slot - available → reserved (valid)",
      async () => {
        ...
        await simulateUserAction(
          "Test slot state transition: available → reserved",
        );
    ...
    await runner.runTest(
      "STATE 2: Slot - reserved → booked (valid)",
      async () => {
        const business = await getRandomBusiness();
        const slot = await getRandomAvailableSlot(business.id);
        cleanup.slots.push(slot.id);
        ...
        await simulateUserAction(
          "Test slot state transition: reserved → booked",
        );
    ...
    await runner.runTest(
      "STATE 3: Slot - reserved → available (valid)",
      async () => {
        const business = await getRandomBusiness();
        const slot = await getRandomAvailableSlot(business.id);
        cleanup.slots.push(slot.id);
        ...
        await simulateUserAction(
          "Test slot state transition: reserved → available",
        );
    ...
    await runner.runTest(
      "STATE 4: Slot - available → booked (invalid)",
      async () => {
        await simulateUserAction(
          "Test invalid slot transition: available → booked",
        );

        if (slotStateMachine.canTransition("available", "book")) {
          throw new Error(
            "State machine incorrectly allows available → booked",
          );
        }
        ...
        console.log(
          `   ✅ Invalid transition correctly rejected: available → booked`,
        );
      },
    );

    await runner.runTest(
      "STATE 5: Slot - booked → reserved (invalid)",
      async () => {
        await simulateUserAction(
          "Test invalid slot transition: booked → reserved",
        );
        ...
        console.log(
          `   ✅ Invalid transition correctly rejected: booked → reserved`,
        );
      },
    );
    ...
    // ============================================
    await runner.runTest(
      "STATE 6: Booking - pending → confirmed (valid)",
      async () => {
        const business = await getRandomBusiness();
        const slot = await getRandomAvailableSlot(business.id);
        cleanup.slots.push(slot.id);
        ...
        await simulateUserAction(
          "Test booking state transition: pending → confirmed",
        );
    ...
    await runner.runTest(
      "STATE 7: Booking - pending → rejected (valid)",
      async () => {
        const business = await getRandomBusiness();
        const slot = await getRandomAvailableSlot(business.id);
        cleanup.slots.push(slot.id);
        ...
        await simulateUserAction(
          "Test booking state transition: pending → rejected",
        );
    ...
    await runner.runTest(
      "STATE 8: Booking - rejected → cancelled (invalid)",
      async () => {
        await simulateUserAction(
          "Test invalid booking transition: rejected → cancelled",
        );

        if (bookingStateMachine.canTransition("rejected", "cancel")) {
          throw new Error(
            "State machine incorrectly allows rejected → cancelled",
          );
        }

        console.log(
          `   ✅ Invalid transition correctly rejected: rejected → cancelled`,
        );
      },
    );
    ...
    // ============================================
    await runner.runTest(
      "STATE 9: Payment - initiated → completed (valid)",
      async () => {
        await simulateUserAction(
          "Test payment state transition: initiated → completed",
        );
    ...
    await runner.runTest(
      "STATE 10: Payment - initiated → failed (valid)",
      async () => {
        await simulateUserAction(
          "Test payment state transition: initiated → failed",
        );
    ...
    await runner.runTest(
      "STATE 11: Payment - completed → failed (invalid)",
      async () => {
        await simulateUserAction(
          "Test invalid payment transition: completed → failed",
        );

        if (paymentStateMachine.canTransition("completed", "fail")) {
          throw new Error(
            "State machine incorrectly allows completed → failed",
          );
        }

        console.log(
          `   ✅ Invalid transition correctly rejected: completed → failed`,
        );
      },
    );

    await runner.runTest(
      "STATE 12: Payment - expired → completed (invalid)",
      async () => {
        await simulateUserAction(
          "Test invalid payment transition: expired → completed",
        );

        if (paymentStateMachine.canTransition("expired", "verify")) {
          throw new Error(
            "State machine incorrectly allows expired → completed",
          );
        }

        console.log(
          `   ✅ Invalid transition correctly rejected: expired → completed`,
        );
      },
    );
    ...
    // ============================================
    await runner.runTest(
      "STATE 13: Direct DB update bypass attempt fails",
      async () => {
        const business = await getRandomBusiness();
        const slot = await getRandomAvailableSlot(business.id);
        cleanup.slots.push(slot.id);
        ...
        await simulateUserAction(
          "Try direct DB update to bypass state machine",
        );
