...
export function runUnitFraudObservabilityTests(): void {
  ...
  runTest("risk score: high cancellation increases score", () => {
    ...
    assert(
      high > low,
      `Higher cancellation should yield higher score: ${low} vs ${high}`,
    );
  });
  ...
  runTest("metrics: observability counter names are non-empty", () => {
    ...
    names.forEach((name) => {
      assert(
        ...
        `Metric name must be non-empty: ${name}`,
      );
    });
  });
  ...
  runTest("alert thresholds: booking success rate min is in [0,1]", () => {
    assert(
      ALERT_BOOKING_SUCCESS_RATE_MIN >= 0 &&
        ALERT_BOOKING_SUCCESS_RATE_MIN <= 1,
      `ALERT_BOOKING_SUCCESS_RATE_MIN should be in [0,1]: ${ALERT_BOOKING_SUCCESS_RATE_MIN}`,
    );
  });

  runTest(
    "fraud caps: attempt rate and accounts per IP caps are positive",
    () => {
      assert(
        FRAUD_BOOKING_ATTEMPT_RATE_CAP > 0,
        "FRAUD_BOOKING_ATTEMPT_RATE_CAP should be > 0",
      );
      assert(
        FRAUD_ACCOUNTS_PER_IP_CAP > 0,
        "FRAUD_ACCOUNTS_PER_IP_CAP should be > 0",
      );
    },
  );
}
