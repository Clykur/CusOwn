...
function createMockRequest(
  ...
  return {
    ...
    cookies: {
      get: (name: string) =>
        name === "csrf-token" && cookieToken
          ? { value: cookieToken }
          : undefined,
    },
    headers: {
      get: (name: string) =>
        name === "x-csrf-token" ? (headerToken ?? null) : null,
    },
  };
}
...
export async function runUnitCsrfTests(): Promise<void> {
  ...
  runTest("should_generateCSRFToken_return_64_char_hex_when_called", () => {
    ...
    assert(
      ...
      `Expected hex string, got '${token.slice(0, 8)}...'`,
    );
  });

  runTest(
    "should_generateCSRFToken_return_different_values_on_each_call",
    () => {
  ...
  runTest(
    "should_validateCSRFToken_return_false_when_POST_and_no_cookie",
    async () => {
  ...
  runTest(
    "should_validateCSRFToken_return_false_when_POST_and_no_header",
    async () => {
  ...
  runTest(
    "should_validateCSRFToken_return_true_when_POST_and_cookie_matches_header",
    async () => {
      const token = generateCSRFToken();
  ...
  runTest("should_setCSRFToken_call_cookies_set_with_token", () => {
    ...
    assert(
      captured.name === "csrf-token",
      `Expected cookie name csrf-token, got ${captured.name}`,
    );
    assert(
      captured.value === "test-token-123",
      `Expected value, got ${captured.value}`,
    );
  });

  runTest(
    "should_csrfProtection_GET_without_cookie_set_cookie_and_return_response",
    async () => {
      const req = createMockRequest("GET");
      ...
      assert(
        (result as any).cookies !== undefined || result !== null,
        "Response returned",
      );
    },
  );
  ...
  runTest(
    "should_csrfProtection_POST_same_origin_valid_token_return_null",
    async () => {
      const token = generateCSRFToken();
      const req = createMockRequest("POST", token, token);
  ...
  runTest(
    "should_csrfProtection_POST_same_origin_no_token_set_and_allow",
    async () => {
  ...
  runTest(
    "should_csrfProtection_POST_same_origin_token_mismatch_return_403",
    async () => {
  ...
  runTest(
    "should_csrfProtection_GET_with_existing_token_return_response",
    async () => {
      const token = generateCSRFToken();
  ...
  runTest(
    "should_csrfProtection_POST_same_origin_via_referer_only",
    async () => {
      const req = createMockRequest("POST", undefined, undefined);
      (req as any).url = "https://example.com/api/action";
      (req as any).headers = {
