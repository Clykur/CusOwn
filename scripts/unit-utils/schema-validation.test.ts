...
async function testSchemaValidation() {
  ...
  try {
    ...
    await runner.runTest(
      "SCHEMA 1: Businesses - Required columns exist",
      async () => {
    ...
    await runner.runTest(
      "SCHEMA 2: Businesses - NOT NULL constraints enforced",
      async () => {
    ...
    await runner.runTest(
      "SCHEMA 3: Businesses - UNIQUE constraint on booking_link",
      async () => {
        ...
        if (
          !e2 ||
          (!e2.message.includes("unique") && !e2.message.includes("duplicate"))
        ) {
    ...
    await runner.runTest(
      "SCHEMA 4: Businesses - UNIQUE constraint on whatsapp_number",
      async () => {
    ...
    await runner.runTest(
      "SCHEMA 5: Businesses - CHECK constraint on slot_duration",
      async () => {
    ...
    // ============================================
    await runner.runTest(
      "SCHEMA 6: Slots - Foreign key to businesses enforced",
      async () => {
        ...
        if (
          ...
          throw new Error(
            "Foreign key constraint not enforced on slots.business_id",
          );
        }

        console.log(
          `   ✅ Foreign key constraint enforced on slots.business_id`,
        );
      },
    );

    await runner.runTest(
      "SCHEMA 7: Slots - CHECK constraint on status",
      async () => {
    ...
    await runner.runTest(
      "SCHEMA 8: Slots - UNIQUE constraint on (business_id, date, start_time, end_time)",
      async () => {
        ...
        // Try duplicate
        const { error: e2 } = await supabase.from("slots").insert(slotData); // Exact duplicate
        ...
        if (
          ...
          throw new Error(
            "UNIQUE constraint not enforced on slots (business_id, date, start_time, end_time)",
          );
        }
    ...
    // ============================================
    await runner.runTest(
      "SCHEMA 9: Bookings - Foreign key to businesses enforced",
      async () => {
        ...
        if (
          ...
          throw new Error(
            "Foreign key constraint not enforced on bookings.business_id",
          );
        }

        console.log(
          `   ✅ Foreign key constraint enforced on bookings.business_id`,
        );
      },
    );

    await runner.runTest(
      "SCHEMA 10: Bookings - Foreign key to slots enforced",
      async () => {
        await simulateUserAction(
          "Test foreign key constraint on bookings.slot_id",
        );
        ...
        if (
          ...
          throw new Error(
            "Foreign key constraint not enforced on bookings.slot_id",
          );
        }

        console.log(
          `   ✅ Foreign key constraint enforced on bookings.slot_id`,
        );
      },
    );

    await runner.runTest(
      "SCHEMA 11: Bookings - UNIQUE constraint on booking_id",
      async () => {
        ...
        if (
          ...
          throw new Error(
            "UNIQUE constraint not enforced on bookings.booking_id",
          );
        }
    ...
    await runner.runTest(
      "SCHEMA 12: Bookings - CHECK constraint on status",
      async () => {
    ...
    // ============================================
    await runner.runTest(
      "SCHEMA 13: Payments - Foreign key to bookings enforced",
      async () => {
        ...
        if (
          ...
          throw new Error(
            "Foreign key constraint not enforced on payments.booking_id",
          );
        }

        console.log(
          `   ✅ Foreign key constraint enforced on payments.booking_id`,
        );
      },
    );

    await runner.runTest(
      "SCHEMA 14: Payments - CHECK constraint on amount_cents",
      async () => {
        ...
        if (
          ...
          throw new Error(
            "CHECK constraint not enforced on payments.amount_cents",
          );
        }
    ...
    await runner.runTest(
      "SCHEMA 15: Payments - UNIQUE constraint on idempotency_key",
      async () => {
        ...
        if (
          ...
          throw new Error(
            "UNIQUE constraint not enforced on payments.idempotency_key",
          );
        }

        console.log(
          `   ✅ UNIQUE constraint enforced on payments.idempotency_key`,
        );
      },
    );
  } finally {
