...
export function runUnitNavigationTests(): void {
  ...
  runTest("ROUTES.BOOKING_STATUS returns path with bookingId", () => {
    ...
    assert(
      out === "/booking/bid-123",
      `Expected '/booking/bid-123', got '${out}'`,
    );
  });
  ...
  runTest("ROUTES.AUTH_LOGIN appends redirect_to when provided", () => {
    ...
    assert(
      out.includes("redirect_to="),
      `Expected redirect_to param, got '${out}'`,
    );
  });
  ...
  runTest("getAdminDashboardUrl appends tab query when provided", () => {
    ...
    assert(
      out === "/admin/dashboard?tab=users",
      `Expected tab param, got '${out}'`,
    );
  });
  ...
  runTest(
    "getOwnerDashboardUrl returns owner path when bookingLink provided",
    () => {
      const out = getOwnerDashboardUrl("my-salon");
      assert(
        out === "/owner/my-salon",
        `Expected '/owner/my-salon', got '${out}'`,
      );
    },
  );
  ...
  runTest("getOwnerDashboardUrl returns dashboard when no bookingLink", () => {
    ...
    assert(
      out === "/owner/dashboard",
      `Expected '/owner/dashboard', got '${out}'`,
    );
  });
}
