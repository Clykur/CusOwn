...
function repeatCustomerPercentage(
...
export function runUnitRecommendationsAnalyticsTests(): void {
  ...
  runTest(
    "recommendation ordering: previously booked business ranks first",
    () => {
      ...
      assert(
        result[0].businessId === "b1",
        "Previously booked (b1) should rank first",
      );
      assert(
  ...
  runTest(
    "recommendation ordering: combined scores rank above single-component",
    () => {
      ...
      assert(
        b1Score > b3Score,
        "Previously-only (b1) should beat nearby-only (b3)",
      );
      assert(
        b2Score > b3Score,
        "Previously+frequent (b2) should beat nearby-only (b3)",
      );
      assert(
        b2Score > b1Score,
        "Previously+frequent (b2) should beat previously-only (b1)",
      );
    },
  );
  ...
  runTest(
    "analytics: repeat customer percentage zero when no customers",
    () => {
