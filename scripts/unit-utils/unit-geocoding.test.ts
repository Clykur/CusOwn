...
async function run() {
  ...
  (global as any).fetch = async (url: string) => {
    if (url.includes("search?")) {
      return {
        ok: true,
        json: async () => [
          { lat: "40.0", lon: "-74.0", display_name: "Test Location" },
        ],
      } as any;
    }
  ...
  assert(
    f1 !== null && f1.lat === 40 && f1.lng === -74,
    "Forward geocode returns coordinates",
  );
  ...
  assert(
    r1 !== null && r1.display_name === "Test Reverse",
    "Reverse geocode returns display_name",
  );
