...
export function runUnitCronAuthTests(): void {
  ...
  runTest(
    "validateCronSecret returns 401 when Authorization header is missing",
    () => {
  ...
  runTest(
    "validateCronSecret returns 401 when Bearer token does not match",
    () => {
      if (!env.cron.secret) return;
      const req = new NextRequest("http://localhost/api/cron/health-check", {
        method: "GET",
  ...
  runTest(
    "validateCronSecret returns null when Bearer token matches configured secret",
    () => {
  ...
  runTest(
    "validateCronSecret returns 401 when header is present but not Bearer",
    () => {
      if (!env.cron.secret) return;
      const req = new NextRequest("http://localhost/api/cron/health-check", {
        method: "GET",
