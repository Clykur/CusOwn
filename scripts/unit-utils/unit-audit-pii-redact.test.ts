...
export function runUnitAuditPiiRedactTests(): void {
  ...
  runTest("redactPiiForAudit redacts customer_name", () => {
    ...
    assert(
      out !== null &&
        (out as Record<string, unknown>).customer_name === "[REDACTED]",
  ...
  runTest("redactPiiForAudit redacts email, phone, name", () => {
    ...
    assert(
      (out as Record<string, unknown>).email === "[REDACTED]",
      "email redacted",
    );
    assert(
      (out as Record<string, unknown>).phone === "[REDACTED]",
      "phone redacted",
    );
    assert(
      (out as Record<string, unknown>).name === "[REDACTED]",
      "name redacted",
    );
  ...
  runTest("redactPiiForAudit redacts nested PII", () => {
    ...
    const meta = (out as Record<string, unknown>).meta as Record<
      string,
      unknown
    >;
    assert(
      meta.customer_email === "[REDACTED]",
      "nested customer_email redacted",
    );
  ...
  runTest("redactPiiForAudit redacts user_agent, ip_address, address", () => {
    ...
    assert(
      (out as Record<string, unknown>).user_agent === "[REDACTED]",
      "user_agent redacted",
    );
    assert(
      (out as Record<string, unknown>).ip_address === "[REDACTED]",
      "ip_address redacted",
    );
    assert(
      (out as Record<string, unknown>).address === "[REDACTED]",
      "address redacted",
    );
    assert(
      (out as Record<string, unknown>).booking_id === "b1",
      "booking_id preserved",
    );
  });
