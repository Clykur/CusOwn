...
export async function runUnitPollWithRetryTests(): Promise<void> {
  ...
  await runAsyncTest(
    "pollWithRetry stops when shouldStop returns true",
    async () => {
      let calls = 0;
  ...
  await runAsyncTest(
    "pollWithRetry stop() prevents further ticks",
    async () => {
      let calls = 0;
      const handle = pollWithRetry({
        fn: async () => ++calls,
  ...
  await runAsyncTest(
    "pollWithRetry invokes onError when fn throws",
    async () => {
      ...
      assert(
        (errorSeen as Error).message === "poll error",
  ...
  await runAsyncTest(
    "pollWithRetry with already aborted signal does not start",
    async () => {
      const ac = new AbortController();
      ac.abort();
      let calls = 0;
