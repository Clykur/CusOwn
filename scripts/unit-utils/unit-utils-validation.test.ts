...
export function runUnitUtilsValidationTests(): void {
  ...
  runTest(
    "should_validateTimeRange_not_throw_when_closing_after_opening",
    () => {
  ...
  runTest(
    "should_validateCreateSalon_return_parsed_data_when_valid_input",
    () => {
      const valid = {
      ...
      assert(
        result.salon_name === valid.salon_name,
        `Expected salon_name, got ${result.salon_name}`,
      );
    },
  );
  ...
  runTest(
    "should_validateCreateBooking_return_parsed_data_when_valid_input",
    () => {
      const valid = {
  ...
  runTest(
    "should_validateCreateBooking_throw_when_customer_name_too_short",
    () => {
      const invalid = {
        salon_id: "11111111-1111-1111-1111-111111111111",
        slot_id: "22222222-2222-2222-2222-222222222222",
  ...
  runTest(
    "should_validateCreateSalon_throw_DATABASE_ERROR_when_parse_throws_non_Error",
    () => {
      ...
      assert(
        msg.includes("Database") || msg.length > 0,
  ...
  runTest(
    "should_validateCreateBooking_throw_DATABASE_ERROR_when_parse_throws_non_Error",
    () => {
  ...
  runTest(
    "should_validateCreateSalon_throw_DATABASE_ERROR_when_catch_receives_non_Error",
    () => {
  ...
  runTest(
    "should_validateCreateBooking_throw_DATABASE_ERROR_when_catch_receives_non_Error",
    () => {
      const proxyThrow = new Proxy({} as any, {
        get: () => {
          throw 123;
