...
export const checkHealth = async (): Promise<HealthStatus> => {
  ...
  const lastRunTsRaw = await safeMetrics.getCount(
    "cron.expire_bookings.last_run_ts",
  );
  const lastRunTs =
    typeof lastRunTsRaw === "number" ? lastRunTsRaw : Number(lastRunTsRaw);
  ...
  await performanceMonitor.recordHealthCheck(
    checks.database === "up",
    duration,
  );
...
export const checkMediaHealth = async (): Promise<{
  ...
  const status =
    checks.media_storage === "up" && checks.media_table === "up"
      ? "healthy"
      : "unhealthy";
  return { status, checks };
};
