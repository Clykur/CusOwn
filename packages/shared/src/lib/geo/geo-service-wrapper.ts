...
import { safeMetrics } from "../monitoring/safe-metrics";
import {
  isGeoCircuitOpen,
  recordGeoFailure,
  recordGeoSuccess,
} from "./geo-cooldown-store";
...
export type GeoLookupOutcome =
  | { ok: true; data: IpLookupResult }
...
export async function ipLookupWithFallback(
  ...
  const cityName = (loc?.city ??
    (data as Record<string, unknown>).locality ??
    "") as string;
  ...
  const country = (data as Record<string, unknown>).country as
    | Record<string, unknown>
    | undefined;
  return {
    ok: true,
