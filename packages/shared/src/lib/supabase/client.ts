...
if (supabaseUrl && supabaseAnonKey) {
  try {
    ...
    console.error(
      "[supabase-client] Failed to initialize Supabase client:",
      error,
    );
  ...
  if (typeof window !== "undefined" && !isPlaceholder) {
    ...
    isPlaceholder &&
    publicEnv.nodeEnv === "development"
  ) {
    // Silent in dev with placeholders to avoid noise, but log a hint
    console.log("[supabase-client] Using development Supabase placeholders.");
  }
}
