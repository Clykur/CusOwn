import { VALIDATION, ERROR_MESSAGES } from "@cusown/config";

export function validateAdminDeletionReason(reason: unknown): string | null {
  if (reason === undefined || reason === null)
    return ERROR_MESSAGES.DELETION_REASON_REQUIRED;
...
export function isReasonRequiredError(message: string): boolean {
  return (
    message.includes("Deletion reason is mandatory") ||
    message.includes("reason is mandatory")
  );
}
