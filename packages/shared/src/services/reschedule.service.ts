...
import { ERROR_MESSAGES, BOOKING_STATUS, SLOT_STATUS } from "@cusown/config";
...
export class RescheduleService {
  ...
  async rescheduleBooking(input: RescheduleInput): Promise<any> {
    ...
    if (!out?.success) {
      ...
      if (err.includes("New slot must be different from current slot")) {
        const latest = await bookingService.getBookingByUuidWithDetails(
          input.bookingId,
        );
        if (!latest) {
          throw new Error(ERROR_MESSAGES.BOOKING_NOT_FOUND);
    ...
    const updated = await bookingService.getBookingByUuidWithDetails(
      input.bookingId,
    );
    if (updated) {
  ...
  async getRescheduleHistory(bookingId: string): Promise<any[]> {
    ...
    const { data, error } = await supabase
      ...
      .select(
        "id, old_slot_id, new_slot_id, action_type, actor_type, created_at",
      )
