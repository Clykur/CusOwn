...
import { canTransition } from "./booking-state-machine.service";
import {
  emitBookingCreated,
...
import { getReviewsByBookingIds } from "./review.service";
...
import { bookingEventsAnalyticsService } from "./booking-events-analytics.service";
import {
  buildQueryCacheKey,
  withQueryCache,
...
export class BookingService {
  ...
  async prepareCreateBookingParams(
    ...
    if (serviceIds && serviceIds.length > 0) {
      ...
      const services = await serviceService.validateServices(
        serviceIds,
        data.salon_id,
      );
  ...
  async createBooking(
    ...
    const params = await this.prepareCreateBookingParams(
      data,
      customerUserId,
      serviceIds,
    );
    return await this.createBookingIdempotentReserve(params);
  }

  async createBookingIdempotentReserve(
    ...
    const bookingWithDetails = await this.getBookingByUuidWithDetails(
      booking.id,
    );
    if (bookingWithDetails) {
      try {
        ...
        console.error(
          "Notification/event failed after booking created:",
          notifyErr,
        );
      }
      ...
      logBookingLifecycle({
        ...
        actor: booking.customer_user_id || "anonymous",
        source: "api",
      });
      if (isQueueAvailable()) {
        void enqueueAnalyticsEvent({
          ...
          eventType: "created",
          actorType: booking.customer_user_id ? "customer" : "system",
  ...
  async getBookingByUuidWithDetails(
    id: string,
  ...
  async confirmBooking(bookingId: string, actorId?: string): Promise<Booking> {
    ...
    if (!allowed) {
      throw new Error(
        `Booking cannot be confirmed from ${booking.status} state`,
      );
    }
    ...
    if (funcError) {
      if (
        String(funcError.message || "").includes("Invalid booking transition")
      ) {
    ...
    if (!result || !result.success) {
      ...
      if (
        errorMsg.includes("Another booking for this slot is already confirmed")
      ) {
    ...
    const bookingWithDetails =
      await this.getBookingByUuidWithDetails(bookingId);
    if (bookingWithDetails) {
  ...
  async rejectBooking(
    ...
    if (!allowed) {
      throw new Error(
        `Booking cannot be rejected from ${booking.status} state`,
      );
    }
    ...
    if (rpcError) {
      if (
        String(rpcError.message || "").includes("Invalid booking transition")
      ) {
        safeMetrics.increment(METRICS_INVALID_STATE_TRANSITION_TOTAL);
      }
  ...
  async revertConfirmToPending(
    ...
    if (!result?.success) {
      ...
      if (
        err.includes("Slot no longer available") ||
        err.includes("Slot update failed")
      ) {
  ...
  async revertRejectToPending(
    ...
    if (!result?.success) {
      ...
      if (
        ...
        (err.includes("not in") || err.includes("not in rejected"))
      ) {
   ...
   */
  private async getBookingsWithDetailsBatch(
    ids: string[],
  ...
  private async _getSalonBookingsUncached(
    ...
    if (
      error &&
      (error.message?.includes("undo_used_at") ||
   ...
   */
  async expireOldBookings(options?: {
    source?: "cron" | "lazy_heal";
  }): Promise<void> {
    const supabaseAdmin = requireSupabaseAdmin();
    ...
    const metricName =
      source === "lazy_heal"
        ? "bookings.expired_by_lazy_heal"
        : "bookings.expired_by_cron";
    ...
    const bookingIds: string[] = Array.isArray(result.booking_ids)
      ? result.booking_ids
      : [];
    ...
    const bookingsWithDetails =
      await this.getBookingsWithDetailsBatch(bookingIds);
  ...
  async cancelBookingByCustomer(
    ...
    if (!cancelAllowed) {
      throw new Error(
        `Booking cannot be cancelled from ${booking.status} state`,
      );
    }
  ...
  private async _getCustomerBookingsUncached(
    ...
    const businessIds = [
      ...new Set(list.map((b: { business_id: string }) => b.business_id)),
    ];
    const slotIds = [
      ...new Set(list.map((b: { slot_id: string }) => b.slot_id)),
    ];
    ...
    const bookingsWithDetails: BookingWithDetails[] = list.map(
      (booking: Booking) => ({
        ...booking,
        salon: salonMap.get(booking.business_id) ?? undefined,
        slot: slotMap.get(booking.slot_id) ?? undefined,
