...
function isMissingConcurrentBookingCapacityError(
  ...
  return (
    m.includes("concurrent_booking_capacity") && m.includes("does not exist")
  );
}
...
export class SalonService {
  ...
  async applyPostCreateSchedule(
    ...
    const existingRows =
      await downtimeService.getBusinessSpecialHours(businessId);
    ...
    if (extra?.closures) {
      for (const c of extra.closures) {
        await downtimeService.addClosure(
          businessId,
          c.start_date,
          c.end_date,
          c.reason,
  ...
  async createSalon(
    ...
    const { weekly_hours, breaks, holidays, closures, services, ...core } =
      data;
    ...
    const capacityRaw =
      core.concurrent_booking_capacity ?? DEFAULT_CONCURRENT_BOOKING_CAPACITY;
    ...
    if (
      insertResult.error &&
      isMissingConcurrentBookingCapacityError(insertResult.error)
    ) {
      logStructured(
        "warn",
    ...
    try {
      ...
      logStructured(
        "warn",
        "Initial slot generation failed; slots will be generated lazily",
        {
          business_id: salon.id,
          error:
  ...
  getSalonByBookingLink = cache(
    async (bookingLink: string): Promise<Salon | null> => {
      // Build cache key for business by booking link
      const cacheKey = buildQueryCacheKey(
        `${QUERY_CACHE_PREFIX.BUSINESS}link:`,
        bookingLink,
        {},
      );
      ...
      return withQueryCache(
        cacheKey,
        QUERY_CACHE_TTL.BUSINESS_PROFILE,
        async () => {
          const supabaseAdmin = requireSupabaseAdmin();
  ...
  getSalonById = cache(
    async (
      ...
      const cacheKey = buildQueryCacheKey(
        `${QUERY_CACHE_PREFIX.BUSINESS}id:`,
        salonId,
        {
          includeSuspended,
          includeDeleted,
