...
import { getClientIp } from "../lib/utils/security.server";
import {
  AUDIT_ACTIONS,
...
export type AuditActionType =
  ...
  | (typeof AUDIT_ACTIONS.REVIEW)[number]
  | (typeof AUDIT_ACTIONS.SERVICE)[number];
...
export function buildAuditDescription(
  ...
  for (const key of keys) {
    ...
    if (
      ...
      changes.push(
        `${label} changed from '${formatVal(oldVal)}' to '${formatVal(newVal)}'.`,
      );
  ...
  if (entityType === "business")
    return `Business ${actionType.replace("business_", "")}.`;
  if (entityType === "user")
...
export class AuditService {
  async createAuditLog(
    ...
    const oldData =
      data.oldData != null
        ? redactPiiForAudit(data.oldData as Record<string, unknown>)
        : null;
    const newData =
      data.newData != null
        ? redactPiiForAudit(data.newData as Record<string, unknown>)
        : null;
    ...
    try {
      ...
      if (data.entityId != null)
        dedupeQuery = dedupeQuery.eq("entity_id", data.entityId);
  ...
  async getAuditLogs(filters?: {
    ...
    if (filters?.businessId) {
      ...
      if (ids.length > 0) {
        ...
        query = query.or(
          `and(entity_type.eq.business,entity_id.eq.${filters.businessId})`,
        );
      }
    }
