...
import { getValidString } from "@cusown/shared/server";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payment =
      await paymentService.getPaymentByPaymentId(validatedPaymentId);
    ...
    if (
      ...
      payment.transaction_id !== validatedTransactionId
    ) {
    ...
    if (payment.status !== "initiated") {
      return errorResponse(
        `Payment must be initiated, found ${payment.status}`,
        400,
      );
    }
    ...
    if (booking.status === BOOKING_STATUS.PENDING) {
      ...
      const bookingWithDetails =
        await bookingService.getBookingByUuidWithDetails(booking.id);
