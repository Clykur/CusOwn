...
import { verifyUPIWebhookSignature, parseUPIWebhookPayload } from '@cusown/shared/server';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!signature) {
      console.warn(`[WEBHOOK] Missing signature from IP: ${sanitizeForLog(clientIP)}`);
    ...
    if (!verifyUPIWebhookSignature(body, signature, secret)) {
      console.warn(`[SECURITY] Invalid UPI webhook signature from IP: ${sanitizeForLog(clientIP)}`);
