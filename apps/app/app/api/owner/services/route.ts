...
export async function GET(request: NextRequest) {
  try {
    ...
    if (businessId) {
      ...
      const cacheKey = buildApiCacheKey('GET', '/api/owner/services', request.nextUrl.searchParams, auth.user.id);
    ...
    if (bookingLink) {
      ...
      const cacheKey = buildApiCacheKey('GET', '/api/owner/services', { bookingLink });
      const cached = getCachedApiResponse<any[]>(cacheKey);
      if (cached) return successResponse(cached);
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create audit log
    await auditService.createAuditLog(auth.user.id, 'service_created', 'service', {
      entityId: service.id,
      newData: service,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Create audit log
    await auditService.createAuditLog(auth.user.id, 'service_updated', 'service', {
      entityId: serviceId,
      oldData: oldService,
      newData: updated,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Create audit log
    await auditService.createAuditLog(auth.user.id, 'service_deleted', 'service', {
      entityId: serviceId,
