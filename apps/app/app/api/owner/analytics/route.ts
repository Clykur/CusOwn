  ...
  try {
    ...
    if (aggregated) {
      ...
      const cacheKey = buildApiCacheKey(
        "GET",
        "/api/owner/analytics",
        { businessId, startDate, endDate, aggregated: "true" },
        auth.user.id,
      );
      ...
      if (cachedData) {
        ...
        setCacheHeaders(
          response,
          AGGREGATED_CACHE_TTL / 1000,
          (AGGREGATED_CACHE_TTL * 2) / 1000,
        );
        return response;
      }
    ...
    if (cachedData) {
      ...
      setCacheHeaders(
        response,
        ANALYTICS_CACHE_TTL / 1000,
        (ANALYTICS_CACHE_TTL * 2) / 1000,
      );
      return response;
    }
    ...
    if (type === "overview") {
      data =
        businessId === "all"
          ...
          : await analyticsService.getBookingAnalytics(
              businessId,
              startDate,
              endDate,
            );
      ...
      data =
        businessId === "all"
          ? await analyticsService.getPeakHoursForBusinesses(
              userBusinessIds,
              startDate,
              endDate,
            )
      ...
      data =
        businessId === "all"
          ? await analyticsService.getCustomerRetentionForBusinesses(
              userBusinessIds,
            )
      ...
      if (businessId === "all") {
        return errorResponse(
          "Advanced analytics requires a single business_id",
          400,
        );
      }
      data = await analyticsService.getOwnerAnalyticsAdvanced(
        businessId,
        startDate,
        endDate,
      );
