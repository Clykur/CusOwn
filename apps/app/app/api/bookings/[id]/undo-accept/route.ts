...
export async function POST(
  ...
  try {
    ...
    const updated = await bookingService.revertConfirmToPending(
      id,
      ctx.user.id,
    );
    try {
    ...
    const response = successResponse(
      updated,
      SUCCESS_MESSAGES.BOOKING_REVERTED_TO_PENDING,
    );
    ...
    console.error(
      `[SECURITY] Undo accept error: IP: ${clientIP}, Error: ${message}`,
    );
