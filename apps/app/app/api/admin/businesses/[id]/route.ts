...
export async function PATCH(
  ...
  try {
    ...
    if (
      filteredBody.salon_name &&
      !validateStringLength(filteredBody.salon_name, 200)
    ) {
      return errorResponse("Salon name is too long", 400);
    }
    if (
      filteredBody.owner_name &&
      !validateStringLength(filteredBody.owner_name, 200)
    ) {
      return errorResponse("Owner name is too long", 400);
    }
    if (
      filteredBody.address &&
      !validateStringLength(filteredBody.address, 500)
    ) {
      return errorResponse("Address is too long", 400);
    }
    if (
      filteredBody.location &&
      !validateStringLength(filteredBody.location, 200)
    ) {
    ...
    if (filteredBody.salon_name !== undefined)
      updateData.salon_name = filteredBody.salon_name;
    if (filteredBody.owner_name !== undefined)
      updateData.owner_name = filteredBody.owner_name;
    if (filteredBody.whatsapp_number !== undefined)
      updateData.whatsapp_number = formatPhoneNumber(
        filteredBody.whatsapp_number,
      );
    ...
    if (filteredBody.address !== undefined)
      updateData.address = filteredBody.address;
    if (filteredBody.location !== undefined)
      updateData.location = filteredBody.location;
    if (filteredBody.suspended !== undefined) {
      updateData.suspended = Boolean(filteredBody.suspended);
      updateData.suspended_at = filteredBody.suspended
    ...
    if (updatedBusiness.owner_user_id && changes.length > 0) {
      try {
        ...
        await adminNotificationService.notifyBusinessOwner(
          updatedBusiness.id,
          message,
        );
      } catch {
