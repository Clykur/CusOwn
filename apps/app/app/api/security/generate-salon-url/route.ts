...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      console.error(`[URL_GEN] Failed to parse request body: ${sanitizeForLog(parseError)}`);
      return errorResponse('Invalid request body', 400);
    }

    const salonId =
      body && typeof body === 'object' && body !== null && 'salonId' in body
        ? (body as { salonId: unknown }).salonId
        : undefined;

    console.log(`[URL_GEN] Received salonId: ${salonId === undefined ? 'missing' : 'provided'}`);
    ...
    try {
      ...
      console.error(`[URL_GEN] Error in getSecureSalonUrl: ${sanitizeForLog(urlError)}`);
      throw urlError;
    }
