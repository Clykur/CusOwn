...
import { Salon, Slot } from "@cusown/shared";
import { formatDate, formatTime } from "@cusown/shared";
import { isTimeInRange } from "@cusown/shared";
...
export default function SalonDetailPage() {
    ...
    const fetchSalon = async () => {
      try {
        ...
        if (isUUID && (!token || token === "pending")) {
          try {
            const { getSecureSalonUrlClient } = await import("@cusown/shared");
        ...
        if (isUUID && (!token || token === "pending")) {
          throw new Error(
            "Access token is required. Please use a valid booking link.",
          );
        }
        ...
        if (!response.ok) {
          ...
          if (response.status === 403) {
            if (isUUID && retryCount < maxRetries) {
              retryCount++;
              try {
                const { getSecureSalonUrlClient } =
                  await import("@cusown/shared");
                const secureUrl = await getSecureSalonUrlClient(salonId);
                if (
                  secureUrl &&
                  secureUrl !== `/salon/${salonId}?token=pending`
                ) {
            ...
            throw new Error(
              "Invalid access token. Please use a valid booking link.",
            );
          }
          throw new Error(
            errorData.error || `Failed to load salon (${response.status})`,
          );
        }
    ...
    const fetchSlots = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          const errorMessage =
            errorData.error || `Failed to load slots (${response.status})`;
        ...
        if (result.success && result.data) {
          if (result.data.closed) {
            ...
            setClosedMessage(null);
            const slotsArr = Array.isArray(result.data)
              ? result.data
              : (result.data.slots ?? []);
            setSlots(slotsArr);
          }
  ...
  useEffect(() => {
    ...
    if (salon?.closing_time) {
      setIsShopClosedToday(
        isShopClosedForSelectedDate(today, salon.closing_time),
      );
    }
              ...
              <div className="space-y-4 pt-6 border-t border-gray-200">
                <div className="bg-gray-50 rounded-xl p-5 border border-gray-200">
                  ...
                  <p className="text-xs text-center text-gray-500 mt-4">
                    By booking, you agree to receive confirmation messages via
                    WhatsApp
                  </p>
                </div>
              </div>
