...
function DeleteAccountSectionComponent({
  profileData,
  setError,
    ...
    <>
      <section className="rounded-xl border border-red-200 bg-red-50/50 p-4 shadow-sm sm:rounded-lg sm:p-6">
        ...
        <p className="text-sm text-slate-600 mb-4">
          Once you delete your account, all your profile information and
          associated business data will be removed from the public platform.
          Your data will be retained for 30 days for administrative purposes
          before being permanently deleted.
        </p>
        {profileData.statistics.businessCount > 0 && (
          <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
            <strong>Warning:</strong> You have{" "}
            {profileData.statistics.businessCount} business
            {profileData.statistics.businessCount > 1 ? "es" : ""} associated
            with your account. Deleting your account will also remove{" "}
            {profileData.statistics.businessCount > 1
              ? "these businesses"
              : "this business"}{" "}
            from the platform.
          </div>
            ...
            <ul className="text-sm text-slate-600 mb-4 list-disc list-inside space-y-1">
              ...
              {profileData.statistics.businessCount > 0 && (
                <li>
                  ...
                  {profileData.statistics.businessCount > 1 ? "es" : ""} from
                  the platform
                </li>
              )}
