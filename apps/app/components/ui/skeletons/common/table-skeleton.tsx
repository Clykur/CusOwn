...
export function TableRowSkeleton({ cols = 4 }: { cols?: number }) {
  return (
    <div className="flex items-center gap-4 p-4 border-b border-border-primary" aria-busy="true">
  ...
  return (
    <div className="space-y-3" aria-busy="true">
      {Array.from({ length: items }).map((_, i) => (
        <div
          key={i}
          className="flex gap-4 p-4 bg-surface-card border border-border-primary rounded-lg"
        >
