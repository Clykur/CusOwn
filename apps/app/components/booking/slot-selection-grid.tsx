...
function SlotSelectionGridComponent({
  ...
  if (isTodayClosed) {
    return (
      <p className="w-full py-3 text-center text-sm leading-relaxed text-text-secondary">
  ...
  if (dateLoading && slots.length === 0) {
    return (
      <div className="flex w-full items-center justify-center gap-2 py-8 text-text-secondary">
        <div className="h-4 w-4 shrink-0 animate-spin rounded-full border-2 border-border-primary border-t-slate-600" />
        ...
        return (
          <button
            ...
            className={`min-h-[2.75rem] w-full min-w-0 rounded-lg border px-1.5 py-2 text-xs font-medium leading-tight transition-colors sm:px-2.5 sm:text-sm ${
              isBooked
                ? 'cursor-not-allowed border-border-primary bg-surface-elevated text-text-secondary'
                : validatingSlot && isSelected
                  ? 'border-amber-300 bg-amber-50 text-amber-900'
                  : isSelected
                    ? 'border-border-primary bg-brand-primary text-text-inverse shadow-sm'
                    : 'border-border-primary bg-surface-card text-text-primary hover:border-border-primary hover:bg-surface-card'
            }`}
          >
            {validatingSlot && isSelected ? (
