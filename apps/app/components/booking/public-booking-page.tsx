    ...
    const debouncedFetch = async () => {
      ...
      try {
        ...
        if (result?.success && result?.data) {
          ...
          if (!rebookAppliedRef.current && isRebookParam) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!isAuthenticated) {
        ...
        const bookingPath = displayBusiness?.id ? `/customer/book/${displayBusiness.id}` : `/customer/book/${businessSlug}`;
  ...
  const breadcrumbItems = useMemo(
    () => [
      { label: 'My Activity', href: '/customer/dashboard' },
      {
        ...
        href: displayBusiness?.id ? `/customer/book/${displayBusiness.id}` : `/customer/book/${businessSlug}`,
      },
    ],
    [displayBusiness?.salon_name, businessSlug, displayBusiness?.id]
  );
    ...
    return (
      <div className="w-full space-y-4 pb-[calc(4.5rem+env(safe-area-inset-bottom,0px))] md:pb-8">
        <Breadcrumb
          items={[
            { label: 'My Activity', href: '/customer/dashboard' },
            { label: 'Loading...', href: displayBusiness?.id ? `/customer/book/${displayBusiness.id}` : `/customer/book/${businessSlug}` },
          ]}
        />
        <BookingPageSkeleton />
      </div>
