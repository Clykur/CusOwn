...
export default function RescheduleButton({
  ...
  useEffect(() => {
    ...
    if (currentSlot) {
      const slotStart = new Date(
        `${currentSlot.date}T${currentSlot.start_time}`,
      );
      const slotEnd = new Date(`${currentSlot.date}T${currentSlot.end_time}`);
      const cutoffTime = new Date(
        slotStart.getTime() - RESCHEDULE_CUTOFF_MINUTES * 60 * 1000,
      );
  ...
  const rescheduleMutation = useOptimisticMutation({
    mutationFn: useCallback(
      async (slotId: string) => {
        const { supabaseAuth } = await import("@cusown/shared");
        const { getCSRFToken } = await import("@cusown/shared");
  ...
  const filteredSlots = useMemo(
    () =>
      availableSlots.filter(
        (slot) =>
          slot.id !== currentSlotRef.current?.id && slot.status === "available",
      ),
    [availableSlots],
  );
    ...
    <>
      {optimisticSlot && isLoading ? (
        <div className="w-full h-11 bg-slate-100 text-slate-600 font-semibold rounded-lg flex items-center justify-center gap-2">
          ...
          <span>
            Rescheduling to {formatTime(optimisticSlot.start_time)}...
          </span>
        </div>
      ) : (
        <button
          ...
          title={
            isWithinCutoffRef.current
              ? "Cannot reschedule within 30 minutes of appointment"
              : ""
          }
