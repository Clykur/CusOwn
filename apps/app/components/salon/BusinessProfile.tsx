...
import { getCachedBusinessProfile, setCachedBusinessProfile } from '@cusown/shared';
...
export const BusinessProfile = () => {
  ...
  const handleBook = (serviceId: string) => {
    if (!salon?.id) return;
    router.push(`/customer/book/${salon.id}?serviceId=${serviceId}`);
  };
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews?business_id=${encodeURIComponent(salon.id)}`, {
      cache: 'no-store',
    })
      .then(async (res) => {
